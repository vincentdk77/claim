...
/*		ServletContext servletContext = getServlet().getServletContext();
		...
		try {
			...
			//yx
			if("3201,3204,3238,3228,3232,3215,3219,3221,3226,3229,3230,3231,3235,3237,3222,3102,3105,3124,3125,3127,3159,3139,3167,3160,3128,3130,3131,3133,3134,3135,3136,3137,3138,3171,3173,3175,3189,3183,3180,3192,3196,3163,3168,3169,3182".indexOf(request.getParameter("riskCode"))>-1){
				templatePath = commonPath
				+ "//Template//template_all_settle.xls";
			}
	...
	private Collection getInfos(HttpServletRequest request){
		...
		try {
			...
			if(!"template".equals(template)){
			  ...
			  //yx Nyxpolicylist字段CALCULATEFLAG为N 清单页面不显示 也不能导出
				String riskCode = request.getParameter("riskCode");
				String sqlCondition="";
			  //if("3201,3204,3215,3219,3221,3226,3229,3230,3231,3222,3102,3105,3124,3125,3127,3128,3130,3131,3132,3133,3134,3135,3136,3137,3138,3171,3173,3175,3189,3192,3196".indexOf(riskCode) > -1){
			//	 sqlCondition = " settleListCode like '"+settleListCode+"'and calculateflag like 'Y' and nodeType like '"+nodeType+"' and validity like '1' order by indexOfSettle ";
			//  }else{
				 sqlCondition = " settleListCode like '"+settleListCode+"' and nodeType like '"+nodeType+"' and validity like '1' order by indexOfSettle ";
			//	  }
				infoes = blNyxsettlelistFacade.findByConditions(sqlCondition);
			}else {
...
//			String kindCode = request.getParameter("kindCode");
				...
				//yx
				String riskCode = request.getParameter("riskCode");
				if("3201,3204,3238,3228,3232,3215,3219,3221,3226,3229,3230,3231,3235,3237,3222,3102,3105,3124,3125,3127,3159,3139,3167,3160,3128,3130,3131,3133,3134,3135,3136,3137,3138,3171,3173,3175,3189,3183,3180,3192,3196,3163,3168,3169,3182".indexOf(riskCode) > -1){
				sqlCondition += "and calculateflag like 'Y'";
			    }
//			sqlCondition += " and kindCode like '"+kindCode+"'";
				...
				infoes = blNyxpolicylistFacade.findByConditions(sqlCondition);
			}
		} catch (Exception e) {
			...
			if ((infos != null) && (infos.size() > 0)) {
				...
				if(!"template".equals(template) && null != planting31Array && planting31Array.length>0){
					...
					for(int indexSettle = 0; indexSettle < planting31Array.length; indexSettle++){
						...
						NyxsettlelistDto nyxsettlelistDto = (NyxsettlelistDto)planting31Array[indexSettle];
						rowIndex++;
						...
						//农户代码
						Label lab = new Label(j, rowIndex, nyxsettlelistDto.getFcode(), wcfF);//j=1
						wrisheet.addCell(lab);
						j++;
						
						//姓名
						lab = new Label(j, rowIndex, nyxsettlelistDto.getFname(), wcfF);//
						wrisheet.addCell(lab);
						j++;
						
						//身份证
						lab = new Label(j, rowIndex, nyxsettlelistDto.getFidcard(), wcfF);
						wrisheet.addCell(lab);
						j++;
						
						//银行账号
						lab = new Label(j, rowIndex, nyxsettlelistDto.getZhibuka(), wcfF);
						wrisheet.addCell(lab);
						j++;

						//联系电话
						lab = new Label(j, rowIndex, nyxsettlelistDto.getPhone(), wcfF);
						wrisheet.addCell(lab);
						j++;
						
						//赔付险别代码
						lab = new Label(j, rowIndex, getKindCodeName(nyxsettlelistDto.getRiskcode(), nyxsettlelistDto.getKindcode()), wcfF);
						wrisheet.addCell(lab);
						j++;
						
						//赔付标的
						lab = new Label(j, rowIndex, getItemCodeName(nyxsettlelistDto.getRiskcode(), nyxsettlelistDto.getItemcode()), wcfF);
						wrisheet.addCell(lab);
						j++;
						
						//剩余面积
						d = round(nyxsettlelistDto.getInsurearea(), 3);
						number = new Number(j, rowIndex, d.doubleValue(), wcfF);
						wrisheet.addCell(number);
						j++;
						
						//剩余保额
						d = round(nyxsettlelistDto.getSuminsured(), 2);
						number = new Number(j, rowIndex, d.doubleValue(), wcfF);
						wrisheet.addCell(number);
						j++;
						
						//受损面积
						d = round(nyxsettlelistDto.getSettlearea(), 3);
						number = new Number(j, rowIndex, d.doubleValue(), wcfF);
						wrisheet.addCell(number);
						j++;
						
						//赔付比例%
						d = round(nyxsettlelistDto.getClaimrate(), 1);
						number = new Number(j, rowIndex, d.doubleValue(), wcfF);
						wrisheet.addCell(number);
						j++;
						
						//损失率%
						d = round(nyxsettlelistDto.getLossrate(), 2);
						number = new Number(j, rowIndex, d.doubleValue(), wcfF);
						wrisheet.addCell(number);
						j++;
						
						//赔偿金额
						d = round(nyxsettlelistDto.getSettlesum(), 2);
						number = new Number(j, rowIndex, d.doubleValue(), wcfF);
						wrisheet.addCell(number);
						j++;
						...
						if(i<planting31Array.length-1){
							int indexNext = i + 1;
							NyxsettlelistDto nyxsettlelistDtoTemp = (NyxsettlelistDto)planting31Array[indexNext];
							String fIdCardTemp = nyxsettlelistDtoTemp.getFidcard();
							if(null != fIdCardTemp && !fIdCardTemp.equals(nyxsettlelistDto.getFidcard())){
						...
						if(needSumByFIdCard){
							...
							BLNyxsettlelistFacade blNyxsettlelistFacade = new BLNyxsettlelistFacade();
							d = round(blNyxsettlelistFacade.getSettleSumByFcodeNodeType(nyxsettlelistDto.getSettlelistcode(), nyxsettlelistDto.getFidcard(), nyxsettlelistDto.getNodetype()), 2);
							number = new Number(j, rowIndex, d.doubleValue(), wcfF);
							wrisheet.addCell(number);
						}
						j++;
						...
						//土地来源
						lab = new Label(j, rowIndex, transformFieldSourceToName(nyxsettlelistDto.getFieldsource()), wcfF);
						wrisheet.addCell(lab);
						j++;
						
						//备注
						lab = new Label(j, rowIndex, nyxsettlelistDto.getRemark(), wcfF);
						wrisheet.addCell(lab);
						i++;
					}
				}else{
					int i=0;
					for(int indexSettle = 0; indexSettle < planting31Array.length; indexSettle++){
						NyxpolicylistDto nyxpolicylistDto = (NyxpolicylistDto) planting31Array[i];
						rowIndex++;
						int j = 1; //j 代表列数。
						...
						//农户代码
						Label lab = new Label(j, rowIndex, nyxpolicylistDto.getFcode(), wcfF);
						wrisheet.addCell(lab);
						j++;

						//姓名
						lab = new Label(j, rowIndex, nyxpolicylistDto.getFname(), wcfF);
						wrisheet.addCell(lab);
						j++;

						//身份证
						lab = new Label(j, rowIndex, nyxpolicylistDto.getFidcard(), wcfF);
						wrisheet.addCell(lab);
						j++;

						//银行账号
						lab = new Label(j, rowIndex, nyxpolicylistDto.getBankcard(), wcfF);
						wrisheet.addCell(lab);
						j++;

						//联系电话
						lab = new Label(j, rowIndex, nyxpolicylistDto.getPhone(), wcfF);
						wrisheet.addCell(lab);
						j++;

						//赔付险别代码
						lab = new Label(j, rowIndex, nyxpolicylistDto.getKindcode(), wcfF);
						wrisheet.addCell(lab);
						j++;

						//赔付标的
						lab = new Label(j, rowIndex, nyxpolicylistDto.getItemcode(), wcfF);
						wrisheet.addCell(lab);
						j++;
						
						//剩余面积
						d = round(nyxpolicylistDto.getAreanumber(), 3);
						number = new Number(j, rowIndex, d.doubleValue(), wcfF);
						wrisheet.addCell(number);
						j++;

						//剩余保额
						d = round(nyxpolicylistDto.getSumamount(), 2);
						number = new Number(j, rowIndex, d.doubleValue(), wcfF);
						wrisheet.addCell(number);
						j++;
						...
						//土地来源
						lab = new Label(j, rowIndex, transformFieldSourceToName(nyxpolicylistDto.getFieldsource()), wcfF);
						wrisheet.addCell(lab);
						j++;

						//备注
						lab = new Label(j, rowIndex, nyxpolicylistDto.getRemark(), wcfF);
						wrisheet.addCell(lab);

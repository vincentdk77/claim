...
public class UITemblorSettleSaveAction0311 extends Action
{
	public ActionForward execute(ActionMapping actionMapping,
		...
		int tempCount = getTemblorSettleListTempCount(timeStamp,registNo);
...
//				String kindCode = httpServletRequest.getParameter("kindCode");
					...
					UIDeleteTemblorSettleListAction0311 uiDeleteTemblor31SettleListAction = new UIDeleteTemblorSettleListAction0311();
					boolean deleteResult = uiDeleteTemblor31SettleListAction.deleteSettleLists(settleListCode, nodeType, riskCode,dbManager);
					if(!deleteResult){
				...
				for(int index=0;index<tempCount;index++){
					...
					blTemblorsettlelistAction.evaluate(dbManager, settleListCode, startIndex, whereCondition);
					startIndex ++;
				}
	...
	private String generateRemark(HttpServletRequest httpServletRequest)throws Exception{
		...
		try {
			...
			remark += temblorSettleExport.getAreaFullName(fAreaCode);
	...
	private int getMaxIndexInSettleList(String settleListCode)throws Exception{
		...
		try {
			...
			int i = blTemblorsettlelistFacade.getMaxIndex(sqlCondition);
			if(0!=i)
				index = i+1;
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	//根据报案号和时间戳获取临时表数据。
	private Collection getTemblorSettleListTemp(String timeStamp, String registNo)throws Exception{
		Collection TemblorSettleList = new ArrayList();
		String sqlCondition = " REGISTCODE = '"+registNo+"' and STRINGTIMESTAMP = '"+timeStamp+"'";
		
		BLTemblorsettlelisttempFacade blTemblor31SettleListTempFacade = new BLTemblorsettlelisttempFacade();
		try {
			TemblorSettleList = blTemblor31SettleListTempFacade.findByConditions(sqlCondition);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return TemblorSettleList;
	}
	//根据报案号和时间戳获取临时表数据。
	private int getTemblorSettleListTempCount(String timeStamp, String registNo)throws Exception{
		int result = 0;
		String sqlCondition = " REGISTCODE = '"+registNo+"' and STRINGTIMESTAMP = '"+timeStamp+"'";
		
		BLTemblorsettlelisttempFacade Temblor31SettleListTempFacade = new BLTemblorsettlelisttempFacade();
		try {
			result = Temblor31SettleListTempFacade.getCount(sqlCondition);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private void deleteTempTableData(String timeStamp, String registNo)throws Exception{
		...
		BLTemblorsettlelisttempFacade blTemblor31SettleListTempFacade = new BLTemblorsettlelisttempFacade();
		try {
			blTemblor31SettleListTempFacade.deleteByConditions(sqlCondition);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}

...
import com.sinosoft.claim.ui.control.viewHelper.RegistViewHelper;
 ...
 */
public class DAACombineRegistViewHelper extends RegistViewHelper {
	private static Log logger = LogFactory.getLog(DAACombineRegistViewHelper.class);
	 ...
	 * 默认构造方法
	 */
	public DAACombineRegistViewHelper() {
	}
...
//add by guolei 3119属于农机具 车险
	...
	public void setRegistDtoView(HttpServletRequest httpServletRequest,
		...
		// 计算出险次数
		getSamePolicyRegistInfo(httpServletRequest, policyNo, registNo,index);
	...
	public void policyDtoToView(HttpServletRequest httpServletRequest,
			String policyNo, String damageDate, String damageHour,int index)
			throws Exception {
		HttpSession session = httpServletRequest.getSession();
...
//		log.debug("要查询的条件conditions=" + conditions);
	...
	public void getSamePolicyRegistInfo(HttpServletRequest httpServletRequest,
			String policyNo, String curRegistNo,int index) throws Exception {
		...
		httpServletRequest.setAttribute("policyNo_"+index, policyNo);
		httpServletRequest.setAttribute("curRegistNo_"+index, curRegistNo);

		httpServletRequest.setAttribute("prpLregistDto1_"+index, prpLregistDto);
		httpServletRequest.setAttribute("prpLregistDto1_"+index, prpLregistDto);
	}
	...
	public void policyListToView(HttpServletRequest httpServletRequest,
		...
		//modify by zhaolu 20060830 start
		 ...
		 if(userDto.getComCode().indexOf("0000") != 0){
			 conditions += " and a.Classcode in ('31','32')";
		 }
		//conditions = conditions + uiPowerInterface.addPower(userDto,"a","","ComCode");
		//modify by zhaolu 20060830 end
		//add by zhaolu 20060815 end
		...
		PageRecord pageRecord = (PageRecord) uiPolicyAction
				.findForRegistConditions(conditions, pageNo, recordPerPage);
		
		
		List registList = (ArrayList) pageRecord.getResult();
		if (registList != null && registList.size() > 0) {
			for (int i = 0; i < registList.size(); i++) {
				PrpCmainDto prpCmainDto = (PrpCmainDto) (((ArrayList) registList)
						.get(i));

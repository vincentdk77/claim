...
public class UIClaimGetSettleListFacade3224 extends Action{
					 ...
					 if(0==countOfSettleMainList){//没有找到保单对应的理赔清单，则把节点序号置-1。在后边根据这个去取保单清单。
						 ...
						 if(null!=kindCode && !"".equals(kindCode))
						  ...
						  if(!"".equals(strSearchCondition))
							 ...
							 PageRecord pageRecordPlantingSettleList = blNyxsettlelistFacade.findByConditions(sqlPlantingSettleList, currentPageNum, rowsPerPage);
							 if(null!=pageRecordPlantingSettleList && pageRecordPlantingSettleList.getCount()>0){
								 int count = blNyxsettlelistFacade.getCount(sqlPlantingSettleList);
								 settleListMap.put("countRecord", count);
								 double sumInsuredArea = blNyxsettlelistFacade.getSum(sqlPlantingSettleList, "INSUREAREA");
								 settleListMap.put("sumInsuredArea", sumInsuredArea);
								 double sumSettleArea = blNyxsettlelistFacade.getSum(sqlPlantingSettleList, "SETTLEAREA");
								 settleListMap.put("sumSettleArea", sumSettleArea);
								 double settleSum = blNyxsettlelistFacade.getSum(sqlPlantingSettleList, "SETTLESUM");
...
//					 sqlCondition += " and REGISTERCODE like '"+claimNo+"'";
				 ...
				 if(0==countOfSettleMainList)//没有找到保单对应的理赔清单，则把节点序号置-1。在后边根据这个去取保单清单。
					 ...
					 while(indexOfNodeType>=0){
						 ...
						 PageRecord pageRecordPlanting31SettleList = blNyxsettlelistFacade.findByConditions(sqlPlantingSettleList, currentPageNum, rowsPerPage);
				  ...
				  if(-1==indexOfNodeType){//-1代表没有理赔清单，需要显示保单最新清单。
					  ...
					  System.err.println("size:"+collectionCmainAgri.size());
					  if(null!=collectionCmainAgri && collectionCmainAgri.size()==1){
						  ...
						  if(null!=collectionInsureMainList && collectionInsureMainList.size()==1){
							  ...
							  if("3201,3204,3238,3232,3215,3219,3221,3226,3229,3230,3231,3235,3237,3222,3102,3105,3124,3125,3127,3159,3139,3167,3160,3128,3130,3131,3133,3134,3135,3136,3137,3138,3171,3173,3175,3189,3183,3180,3192,3196,3228,3163,3168,3169,3182".indexOf(policyNo.substring(1, 4)) > -1){
							  sqlInsureMainList += " and calculateflag like 'Y' ";
							  }
							  //加入保单清单有效性校验！
							  ...
							  PageRecord pageRecordPlantingPolicyList = blNyxpolicylistFacade.findByConditions(sqlInsureMainList, currentPageNum, rowsPerPage);
							  if(null!=pageRecordPlantingPolicyList && pageRecordPlantingPolicyList.getCount()>0){
								  int count = blNyxpolicylistFacade.getCount(sqlInsureMainList);
								  settleListMap.put("countRecord", count);
								  double sumInsuredArea = blNyxpolicylistFacade.getSum(sqlInsureMainList,"AREANUMBER");
								  settleListMap.put("sumInsuredArea", sumInsuredArea);
...
//				  settleListMap.put("kindCodesList", kindCodesList);
			  ...
			  if("loadXLS".equals(loadXLS)){//导入清单时，从临时表查找数据显示。
				  ...
				  PageRecord pageRecordPlantingSettleList = blNyxsettlelisttempFacade.findByConditions(sqlCondition, currentPageNum, rowsPerPage);
				  if(null!=pageRecordPlantingSettleList && pageRecordPlantingSettleList.getCount()>0){
					  Collection collectionListTable = pageRecordPlantingSettleList.getResult();
					  int count = blNyxsettlelisttempFacade.getCount(sqlCondition);
					  if(count>0){
						 totalPageNum = (count-1)/rowsPerPage+1;
					  }else{
						 totalPageNum = 0;
					  ...
					  double sunInsuredArea = blNyxsettlelisttempFacade.getSum(sqlCondition, "INSUREAREA");
					  settleListMap.put("sumInsuredArea", sunInsuredArea);
					  double sumSettleArea = blNyxsettlelisttempFacade.getSum(sqlCondition, "SETTLEAREA");
					  settleListMap.put("sumSettleArea", sumSettleArea);
					  double settleSum = blNyxsettlelisttempFacade.getSum(sqlCondition, "SETTLESUM");
					  settleListMap.put("settleSum", settleSum);
					  settleListMap.put("countRecord", count);
					  settleListMap.put("currentPageNum", currentPageNum);
					  settleListMap.put("totalPageNum", totalPageNum);

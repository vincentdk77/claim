...
public class UIPlanting31SettleSaveAction extends Action
{
...
//				String kindCode = httpServletRequest.getParameter("kindCode");
					UIDeletePlanting31SettleListAction uiDeletePlanting31SettleListAction = new UIDeletePlanting31SettleListAction();
				...
				for(int index=0;index<tempCount;index++){
					...
					blPlanting31SettleListAction.evaluate(dbManager, settleListCode, startIndex, whereCondition);
					startIndex ++;
				}
	...
	private int getMaxIndexInSettleList(String settleListCode)throws Exception{
		...
		try {
			...
			int i = blPlanting31SettleListFacade.getMaxIndex(sqlCondition);
			if(0!=i)
				index = i+1;
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	private int getPlantingSettleListTempCount(String timeStamp, String registNo)throws Exception{
		int result = 0;
		String sqlCondition = " REGISTCODE = '"+registNo+"' and STRINGTIMESTAMP = '"+timeStamp+"'";
		BLPlanting31SettleListTempFacade Planting31SettleListTempFacade = new BLPlanting31SettleListTempFacade();
		try {

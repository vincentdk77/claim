 ...
 */
public class DBPrpJpayRec extends DBPrpJpayRecBase {
	/**
	 * 构造函数
	 * 
	 * @param dbManager
	 *            资源管理类
	 */
	public DBPrpJpayRec(DBManager dbManager) {
		super(dbManager);
	}
	...
	public int checkPay(String conditions) throws Exception {
		...
		if(riskcode.trim().startsWith("23220")) {
			String check3220 = "select count(*) from prpcplan where " + conditions +" and payreason='R10' and planfee=0";
	...
	public int[] getDelinquentfeeTime(String conditions) throws Exception {
		String statement = "SELECT serialno FROM prpcplan where payno > 0 and delinquentfee >0 and ("
				+ conditions + ")";

 ...
 */
public class DBPrpCinsuredBase{
    protected  DBManager dbManager = null; //资源管理类
    private static Log log = LogFactory.getLog(DBPrpCinsuredBase.class.getName());

    /**
     * 构造函数
     * @param dbManager 资源管理类
     */
    public DBPrpCinsuredBase(DBManager dbManager){
    ...
    public void insert(PrpCinsuredDto prpCinsuredDto) throws Exception{
        String statement = " Insert Into PrpCinsured(" + 
                           " PolicyNo," + 
                           " RiskCode," + 
                           " SerialNo," + 
                           " Language," + 
    ...
    public void delete(String policyNo,int serialNo) throws Exception{
        String statement = " Delete From PrpCinsured Where " +
                           " PolicyNo = ? And " + 
                           " SerialNo = ?";
        log.debug(statement);
        dbManager.prepareStatement(statement);
    ...
    public void update(PrpCinsuredDto prpCinsuredDto) throws Exception{
        String statement = " Update PrpCinsured Set RiskCode = ?," + 
                           " Language = ?," + 
                           " InsuredType = ?," + 
                           " InsuredCode = ?," + 
                           " InsuredName = ?," + 
    ...
    public Collection findByConditions(String conditions,int pageNo,int rowsPerPage) throws Exception{
        String statement = "Select PolicyNo," + 
                           ...
                           " BenefitFlag," + 
                           " BenefitRate," + 
                           " Flag From PrpCinsured Where " + conditions;
        log.debug(statement);
    ...
    public Collection findVirturlItemByConditions(String conditions,int pageNo,int rowsPerPage) throws Exception{
    	...
    	blPrpCvirturlItem.query(conditions);
    	//获取机构代码
    	if(blPrpCvirturlItem.getSize() > 0) {
    		comCodeSH = blPrpCvirturlItem.getArr(0).getPolicyNo().substring(5, 7);		
    	}
    	//如果是上海机构不让查询出虚拟分户
    	String statement = "";
    	if(comCodeSH.equals("31")){
    		statement = "select distinct cv.policyno,cv.riskcode,cv.familyno,cv.familyname,cv.identifynumber,cv.familyage,cv.familysex,cv.benname,cv.bensex from prpcvirturlitem cv where " + conditions + "  and virturlflag <> 1 ";
    	}else{
    		statement = "select distinct cv.policyno,cv.riskcode,cv.familyno,cv.familyname,cv.identifynumber,cv.familyage,cv.familysex,cv.benname,cv.bensex from prpcvirturlitem cv where " + conditions;
    	}
		log.debug(statement);
        Collection collection = new ArrayList();
    ...
    public int deleteByConditions(String conditions) throws Exception{
        String statement = "Delete From PrpCinsured Where " + conditions;
        log.debug(statement);
    ...
    public int getCount(String conditions) 
        throws Exception{
        int count = -1;
        String statement = "Select count(*) from PrpCinsured Where " + conditions;
     ...
     * @throws Exception
     */
    public Collection findByPlantingConditions(String conditions,int pageNo,int rowsPerPage) throws Exception{
        String statement = "Select " + 
                           "i.policyno, "+
                           "t.indexcode, "+
                           "t.fname, "+
                           "t.fidcard "+
                           "FROM INSUREMAINLIST I,PLANTINGPOLICYLIST T "+
                           "Where "+conditions+
                           "union "+
                           "Select " + 
                           "i.policyno, "+
                           ...
                           "t.fname, "+
                           "t.fidcard "+
                           "FROM INSUREMAINLIST I,planting31policylist T "+
                           "Where "+conditions+
                           "union "+
                           "Select " + 
                           "i.policyno, "+
                           ...
                           "t.fname, "+
                           "t.fidcard "+
                           "FROM INSUREMAINLIST I,nyxpolicylist T "+
                           "Where "+conditions;
        log.debug(statement);
        Collection collection = new ArrayList();
        PrpCinsuredDto prpCinsuredDto = null;
        ...
        while(resultSet.next()){
            ...
            prpCinsuredDto = new PrpCinsuredDto();
            prpCinsuredDto.setPolicyNo(dbManager.getString(resultSet,1));
            prpCinsuredDto.setSerialNo(dbManager.getInt(resultSet,2));
            prpCinsuredDto.setInsuredName(dbManager.getString(resultSet,3));
            prpCinsuredDto.setIdentifyNumber(dbManager.getString(resultSet,4));
            collection.add(prpCinsuredDto);
        }
        resultSet.close();
        log.info("DBPrpCinsuredBase.findByConditions() success!");

 ...
 */
public class DBPrpLchargeBaseFG{
    protected DBManager dbManager = null; //资源管理类
    private static Log logger = LogFactory.getLog(DBPrpLchargeBaseFG.class);

    /**
     * 构造函数
     * @param dbManager 资源管理类
     */
    public DBPrpLchargeBaseFG(DBManager dbManager){
    ...
    public void insert(PrpLchargeDto prpLchargeDto) throws Exception{
        String mainStatement = " Insert Into PrpLcharge (" + 
                           " CompensateNo," + 
                           " RiskCode," + 
                           " PolicyNo," + 
                           " SerialNo," + 
    ...
    public void delete(String compensateNo,int serialNo) throws Exception{
        String statement = " Delete From PrpLcharge" + 
	            		   " Where " +
                           " CompensateNo = ? And " + 
                           " SerialNo = ?";
        if(logger.isDebugEnabled()){
            String debugStatement = "Delete From PrpLcharge Where " +
    ...
    public void update(PrpLchargeDto prpLchargeDto) throws Exception{
        String statement = " Update PrpLcharge Set RiskCode = ?," + 
                           " PolicyNo = ?," + 
                           " KindCode = ?," + 
                           " ChargeCode = ?," + 
                           " ChargeName = ?," + 
                           ...
                           " SerialNo = ?";
        if(logger.isDebugEnabled()){
            String debugStatement = "Update PrpLcharge Set " + 
    ...
    public Collection findByConditions(String conditions,int pageNo,int rowsPerPage) throws Exception{
        String statement = "Select CompensateNo," + 
                           ...
                           " preChargeAmount," +
                           " checkDeptCode," +
                           " checkDeptName From PrpLcharge Where " + conditions;
        ...
        while(resultSet.next()){
            ...
            prpLchargeDto.setPolicyNo(dbManager.getString(resultSet,3));
            prpLchargeDto.setSerialNo(dbManager.getInt(resultSet,4));
            prpLchargeDto.setKindCode(CodeTransfer.FGKindKodeOut.covrentPlatFormCode(dbManager.getString(resultSet,5)));
            prpLchargeDto.setChargeCode(dbManager.getString(resultSet,6));
            prpLchargeDto.setChargeName(dbManager.getString(resultSet,7));
            prpLchargeDto.setCurrency(dbManager.getString(resultSet,8));
            prpLchargeDto.setChargeAmount(dbManager.getDouble(resultSet,9));
    ...
    public int deleteByConditions(String conditions) throws Exception{
        String statement = "Delete From PrpLcharge Where " + conditions;
        logger.debug(statement);
    ...
    public int getCount(String conditions) 
        throws Exception{
        int count = -1;
        String statement = "Select count(1) from PrpLcharge Where " + conditions;
    ...
    throws Exception{
    int count = -1;
    String statement = "Select max(SerialNo) from PrpLcharge Where " + conditions;
    statement = SqlUtils.getWherePartForGetCount(statement);
    logger.debug(statement);
    ResultSet resultSet = dbManager.executeQuery(statement);
    resultSet.next();

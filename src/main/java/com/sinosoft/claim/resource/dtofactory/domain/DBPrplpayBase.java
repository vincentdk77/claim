 ...
 */
public class DBPrplpayBase{
    /**
     * 资源管理类的实例，处理数据库操作.
     */
    protected DBManager dbManager = null;
    private static Logger logger = Logger.getLogger(DBPrplpayBase.class);

    /**
     * 构造函数
     * @param dbManager 资源管理类
     */
    public DBPrplpayBase(DBManager dbManager){
    ...
    public void insert(PrplpayDto prplpayDto)
        ...
        buffer.append("INSERT INTO Prplpay (");
    ...
    public void delete(String serialno,String compensateno,String serialno2)
        ...
        buffer.append("DELETE FROM Prplpay ");
        if(logger.isDebugEnabled()){
            StringBuffer debugBuffer =  new StringBuffer(buffer.length()*4);
            debugBuffer.append(buffer.toString());
...
//    }
    ...
    public void update(PrplpayDto prplpayDto)
            throws Exception{
        StringBuffer buffer = new StringBuffer(200);
        buffer.append("UPDATE Prplpay SET ");
        buffer.append("Claimno = ?, ");
        buffer.append("Classcode = ?, ");
        buffer.append("Riskcode = ?, ");
        buffer.append("Registno = ?, ");
        ...
        if(logger.isDebugEnabled()){
            StringBuffer debugBuffer =  new StringBuffer(buffer.length()*4);
            debugBuffer.append("UPDATE Prplpay SET ");
...
//    }
    ...
    public Collection findByConditions(String conditions,int pageNo,int rowsPerPage)
        ...
        buffer.append("FROM Prplpay p left join prplcompensate c on p.compensateno=c.compensateno left join prplprepay pre on p.compensateno=pre.precompensateno join prplpaymain paymain on p.serialno=paymain.paymentNo WHERE ");
    ...
    public Collection getSumPayamount(String conditions)
        ...
        StringBuffer buffer = new StringBuffer(200);
        //拼SQL语句
        buffer.append("select p.compensateno,p.paymenttype,p.coinscode,SUM(p.payamount) AS payamount FROM prplpay p WHERE ");
        buffer.append(conditions);
    ...
    public int deleteByConditions(String conditions)
            throws Exception{
        StringBuffer buffer = new StringBuffer(100);
        buffer.append("DELETE FROM Prplpay WHERE ");
        buffer.append(conditions);
        if(logger.isDebugEnabled()){
            logger.debug(buffer.toString());
        }
    ...
    public int getCount(String conditions) 
        ...
        int count = -1;
        StringBuffer buffer = new StringBuffer(100);
        buffer.append("SELECT count(p.serialno) FROM Prplpay p left join prplcompensate c on p.compensateno=c.compensateno left join prplprepay pre on p.compensateno=pre.precompensateno WHERE ");
        buffer.append(conditions);
        if(logger.isDebugEnabled()){
            logger.debug(buffer.toString());
        }
    ...
    public int deleteByConditionsByPrepered(String conditions)
            throws Exception{
        StringBuffer buffer = new StringBuffer(100);
        buffer.append("DELETE FROM Prplpay WHERE serialno = ? ");
        if(logger.isDebugEnabled()){
            logger.debug(buffer.toString());
        }
        dbManager.prepareStatement(buffer.toString());

...
public class DBCertifyCombine
{
    ...
    throws SQLException,Exception
  {
   String nodeType = certifyDto.getNodeType();
    ...
    new DBPrpLcertifyCollect(dbManager).insertAll(certifyDto.getPrpLcertifyCollectDtoList());
    ...
    throws SQLException,Exception
 {
    ...
    if(nodeType.equals("certi")){
      if(certifyDto.getPrpLclaimStatusDtoList()!=null){
    	  ArrayList PrpLclaimStatusDtoList = certifyDto.getPrpLclaimStatusDtoList();
    	  for(int i=0;i<PrpLclaimStatusDtoList.size();i++){
    		 PrpLclaimStatusDto prpLclaimStatusDto = (PrpLclaimStatusDto)PrpLclaimStatusDtoList.get(i);
    	     String condition3 = " BusinessNo='" + prpLclaimStatusDto.getBusinessNo().trim() + "' "+ " AND NodeType ='certi'";
             statement = " DELETE FROM prpLclaimStatus Where " + condition3;
             dbManager.executeUpdate(statement);
             new DBPrpLclaimStatus(dbManager).insert(prpLclaimStatusDto);
    	  }
      }
    }
  }

 ...
 */
public class DBPrplpay extends DBPrplpayBase{
    private static Logger logger = Logger.getLogger(DBPrplpay.class);

    /**
     * 构造函数
     * @param dbManager 资源管理类
     */
    public DBPrplpay(DBManager dbManager){
        super(dbManager);
    }
    /**
     * 更新支付标记，
	...
	* 审核占用必须是待审核状态
    ...
    throws Exception{
        ...
        buffer.append("WHERE ");
        //buffer.append("registno = ? And ");
        buffer.append("serialno = ?  and vflag='6' ");
        dbManager.prepareStatement(buffer.toString());
        //设置更新字段;
        dbManager.setString(1,vflag);
        //设置条件字段;
...
//        buffer.append("')");
	...
	public Collection findByConditionsSql(String conditions) throws Exception{
        ...
        buffer.append("FROM Prplpay where ");

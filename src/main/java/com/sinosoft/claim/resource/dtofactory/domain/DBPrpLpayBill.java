 ...
 */
public class DBPrpLpayBill extends DBPrpLpayBillBase{
    private static Logger logger = Logger.getLogger(DBPrpLpayBill.class);

    /**
     * 构造函数
     * @param dbManager 资源管理类
     */
    public DBPrpLpayBill(DBManager dbManager){
        super(dbManager);
    }
	...
	public Collection findByPaymentNo(String conditions) throws Exception {
        ...
        buffer.append("FROM PrpLpayBill,prplpaymain  WHERE ");
    ...
    /*********************优化修改：end**********************/
    public PrpLpayBillDto findByBillFlag(String registNo,String compensateNo,int serialNo)
    throws Exception{
		StringBuffer buffer = new StringBuffer(200);
		//拼SQL语句
		buffer.append("SELECT ");
		...
		buffer.append("WHERE exists (select 1 from prplpaymain where prplpaymain.paymentno=prplpaybill.paymentno and prplpaymain.billflag='1') and ");
		buffer.append("registNo = ? And ");
		buffer.append("compensateNo = ? And ");
		buffer.append("serialNo = ?");

...
import com.sinosoft.claim.resource.dtofactory.domain.DBPrpCaddressTg;
...
import com.sinosoft.claim.resource.dtofactory.domain.DBPrpCcoinsTg;
import com.sinosoft.claim.resource.dtofactory.domain.DBPrpCengage;
import com.sinosoft.claim.resource.dtofactory.domain.DBPrpCengageTg;
import com.sinosoft.claim.resource.dtofactory.domain.DBPrpCfee;
import com.sinosoft.claim.resource.dtofactory.domain.DBPrpCfeeTg;
import com.sinosoft.claim.resource.dtofactory.domain.DBPrpCinsured;
import com.sinosoft.claim.resource.dtofactory.domain.DBPrpCinsuredTg;
...
import com.sinosoft.claim.resource.dtofactory.domain.DBPrpCitemKindTg;
...
import com.sinosoft.claim.resource.dtofactory.domain.DBPrpCmainLoanTg;
import com.sinosoft.claim.resource.dtofactory.domain.DBPrpCmainOriginTg;
...
import com.sinosoft.claim.resource.dtofactory.domain.DBPrpCplanTg;
...
//add by lym 20060204  for 强三 ---- end   >>>>>>   
   ...
   *@return 无
   */
  public PolicyDto findTGByPrimaryKey(DBManager dbManager,String policyNo)
    throws SQLException,Exception
  {
     PolicyDto policyDto = new PolicyDto();
     ...
     //取得涉案车辆
     com.sinosoft.claim.resource.dtofactory.domain.DBPrpCmainTg dbPrpCmainTg = new com.sinosoft.claim.resource.dtofactory.domain.DBPrpCmainTg(dbManager);
     PrpCmainDto prpCmainDto = dbPrpCmainTg.findByPrimaryKey(policyNo);
     policyDto.setPrpCmainDto(prpCmainDto);
     policyDto.setPrpCfeeDto(new DBPrpCfeeTg(dbManager).findByPrimaryKey(policyNo,"CNY"));
     String conditions = " policyNo = '" + policyNo + "'"; 
     policyDto.setPrpCitemKindDtoList((ArrayList)new DBPrpCitemKindTg(dbManager).findByConditions(conditions,0,0));
     policyDto.setPrpCaddressDtoList((ArrayList)new DBPrpCaddressTg(dbManager).findByConditions(conditions,0,0));
     policyDto.setPrpCinsuredDtoList((ArrayList)new DBPrpCinsuredTg(dbManager).findByConditions(conditions,0,0));
     policyDto.setPrpCitemCarDtoList((ArrayList)new DBPrpCitemCar(dbManager).findByConditions(conditions,0,0));
     policyDto.setPrpCprofitDetailDtoList((ArrayList)new DBPrpCprofitDetail(dbManager).findByConditions(conditions,0,0));
     policyDto.setPrpCprofitDtoList((ArrayList)new DBPrpCprofit(dbManager).findByConditions(conditions,0,0));
     policyDto.setPrpCplanDtoList((ArrayList)new DBPrpCplanTg(dbManager).findByConditions(conditions,0,0));
     policyDto.setPrpCengageDtoList((ArrayList)new DBPrpCengageTg(dbManager).findByConditions(conditions,0,0));
     policyDto.setPrpCfeeDtoList((ArrayList)new DBPrpCfeeTg(dbManager).findByConditions(conditions,0,0));
     policyDto.setLiabStartDate((DateTime)new DBPrpCmainLiab1(dbManager).findByPrimaryKeyStartDate(conditions));
     //取得驾驶员信息
     policyDto.setPrpCcarDriverDtoList((ArrayList)new DBPrpCcarDriver(dbManager).findByConditions(conditions,0,0));
     policyDto.setPrpLclaimStatusDto(new DBPrpLclaimStatustg(dbManager).findByPrimaryKey(policyNo,"polic",0));
     
     //add by miaowenjun 20060430 取得个贷险房屋信息
     policyDto.setPrpCitemHouseDtoList((ArrayList)new DBPrpCitemHouse(dbManager).findByConditions(conditions,0,0));
     policyDto.setPrpCmainLoanDtoList(new DBPrpCmainLoanTg(dbManager).findByPrimaryKey(policyNo));
     //add by miaowenjun 20060430
     
     //modify by wangwei add start 20050601
     policyDto.setPrpCmainCargoDto(new DBPrpCmainCargo(dbManager).findByPrimaryKey(policyNo));
     //modify by wangwei add end 20050601
     policyDto.setPrpCmainOriginDto(new DBPrpCmainOriginTg(dbManager).findByPrimaryKey(policyNo));
     //modify by liuyanmei add start 20050601
        
     policyDto.setPrpClimitDtoList((ArrayList)new DBPrpClimit(dbManager).findByConditions(conditions,0,0));
     policyDto.setPrpCcoinsDtoList((ArrayList)new DBPrpCcoinsTg(dbManager).findByConditions(conditions,0,0));
     policyDto.setPrpCcoinsDetailDtoList((ArrayList)new DBPrpCcoinsDetail(dbManager).findByConditions(conditions,0,0));
     //modify by liuyanmei add end 20050601 

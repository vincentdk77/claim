 ...
 */
public class DBPrpLclaimBillManagertg extends DBPrpLclaimBillManagertgBase{
     ...
     */
    public DBPrpLclaimBillManagertg(DBManager dbManager){
        super(dbManager);
    }
	...
	public Collection findByConditonBillSum(String conditions,String editType,int rowsPerPage,int pageNo) throws Exception {
		...
		buffer.append("nn.payremark ");
		buffer.append(")");
		buffer.append("group by registNo,claimNo, policyNo, receiverfullname, certifno,provincecode,citycode,accounttype,accountflag,banktype,bank,bankaccount,paymentno,mobilephone,address,vFlag,payWay,payReason,payPurpose,payremark ");
		if(!"SHOW".equals(editType)){
			buffer.append("  having sum(payAmount)-sum(havPay)<>0 ");
		}
		System.err.println(buffer.toString());

 ...
 * 这是PRPLPAY的业务逻辑对象Facade基类<br>
 */
public class BLPrplpayFacadeBase{
    private static Logger logger = Logger.getLogger(BLPrplpayFacadeBase.class);

    /**
     * 构造函数
     */
    public BLPrplpayFacadeBase(){
    }
...
//    }
    ...
    public void delete(String serialno,String compensateno,String serialno2)
        ...
        try{
            ...
            dbManager.beginTransaction();
            //删除记录
            blPrplpayAction.delete(dbManager,serialno, compensateno, serialno2);
            dbManager.commitTransaction();
        }catch(Exception exception){
            dbManager.rollbackTransaction();
            throw exception;
...
//    }
    ...
    public Collection getSumPayamount(String conditions)
        ...
        try{
        	dbManager.open(AppConfig.get("sysconst.DBJNDI")) ;
            collection = blPrplpayAction.getSumPayamount(dbManager,conditions);
        }catch(Exception exception){
            throw exception;
        }finally{
            dbManager.close();

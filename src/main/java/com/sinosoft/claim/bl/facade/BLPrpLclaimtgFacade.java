...
/**
 * 这是PrpLclaim-立案基本信息表的业务对象tgFacade类<br>
 * 创建于 2005-03-18 17:08:11.859<br>
 * JToolpad(1.3.3) Vendor:zhouxianli1978@hotmail.com
 */
public class BLPrpLclaimtgFacade extends BLPrpLclaimtgFacadeBase{
    private static Log logger = LogFactory.getLog(BLPrpLclaimtgFacade.class);

    /**
     * 构造函数
     */
    public BLPrpLclaimtgFacade(){
        super();
    }
    /**
     * 取此保单离出险日期最近的危险单位的比例
    ...
    {
        BLPrpCdangerUnitFacade blPrpCdangerUnittgFacade =  new  BLPrpCdangerUnitFacade();
        BLPrpPdangerUnitFacade blPrpPdangerUnittgFacade =  new BLPrpPdangerUnitFacade();
        ...
        BLPrpPheadFacade blPrpPheadtgFacade  = new BLPrpPheadFacade();
        ...
        if(ednorserTimes>0)
        {
     	      ...
     	      prpPheadList = (ArrayList)blPrpPheadtgFacade.findByConditions(sql);
     	      ...
     	      if("".equals(endorseno))//出险日期之前没有发生过批改,直接查prpcdangerunit
     	      {
         	     ...
         	     prpCdangerUnitList =  (ArrayList)blPrpCdangerUnittgFacade.findByConditions(sql);
     	      }else
     	      {
     	         //select * from prppdangerunit pd where pd.endorseno='+endorseNo+';
     		     sql = "endorseno='"+endorseno+"'";
     		     prpPdangerUnitList = (ArrayList)blPrpPdangerUnittgFacade.findByConditions(sql);
     	      }
        }else
        {
     	     sql = "policyno='"+policyno+"'";

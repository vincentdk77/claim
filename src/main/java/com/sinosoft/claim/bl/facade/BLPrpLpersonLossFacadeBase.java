 ...
 */
public class BLPrpLpersonLossFacadeBase{
    private static Log logger = LogFactory.getLog(BLPrpLpersonLossFacadeBase.class);

    /**
     * 构造函数
     */
    public BLPrpLpersonLossFacadeBase(){
    }
    ...
    public Collection findByRecase(String conditions) throws Exception{
        ...
        try{
            dbManager.open(AppConfig.get("sysconst.DBJNDI"));
            collection = blPrpLpersonLossAction.findByRecase(dbManager,conditions);
        }catch(Exception exception){
            throw exception;
        }finally{
            dbManager.close();

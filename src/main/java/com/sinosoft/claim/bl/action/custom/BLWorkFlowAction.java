 ...
 * @version 1.0
 */
public class BLWorkFlowAction
{
  private static Log logger = LogFactory.getLog(BLWorkFlowAction.class);

    /**
     * 构造函数
     */
    public BLWorkFlowAction(){
    }
 ...
 */
    ...
    public void deal(DBManager dbManager,WorkFlowDto workFlowDto)
       ...
       {
         createFlow(dbManager,workFlowDto);
       }
      //     重开工作流程
       if (workFlowDto.getReOpen() )
       {
       	reOpenFlow(dbManager,workFlowDto);
       }
       //提交-工作流 (先提交，后修改？？)
       if (workFlowDto.getSubmit() )
       {
         submitNode(dbManager,workFlowDto);
       }
       //判断是不是释放所有占号的操作
       if (workFlowDto.getFreeHoldNode () )
       {
         freeAllHoldNode(dbManager,workFlowDto);
       }
       //修改工作流
       if (workFlowDto.getUpdate() )
       {
         updateNode(dbManager,workFlowDto);
       }
       //关闭-工作流
       if (workFlowDto.getClose() )
       {
    	   closeAndStoreFlow(dbManager,workFlowDto);
       }
       //收回-工作流
       if (workFlowDto.getRecycle() )
       {
         recycleFlow(dbManager,workFlowDto);
       }
    }

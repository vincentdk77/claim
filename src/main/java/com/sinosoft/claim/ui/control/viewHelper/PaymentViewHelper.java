...
import com.sinosoft.claim.bl.facade.BLPrpLpayExttgFacade;
import com.sinosoft.claim.bl.facade.BLPrpLpayHisFacade;
import com.sinosoft.claim.bl.facade.BLPrpLpayHistgFacade;
...
//	    	}
	...
	public void paymainPostEditFromUndwrt(HttpServletRequest request) throws Exception{
        ...
        //add 托管险种  2018年8月11日11:29:07
        if(prpLpaymainDto==null){
            BLPrpLpayExttgFacade blPrpLpayExttgFacade = new BLPrpLpayExttgFacade();
    		BLPrpLpaymaintgFacade blPrpLpaymaintgFacade = new BLPrpLpaymaintgFacade();
    		BLPrplpaytgFacade blPrplpaytgFacade = new BLPrplpaytgFacade();
    		ArrayList<PrpLpayExtDto> prpLpayExttgDtoList = new ArrayList<PrpLpayExtDto>();
            PrpLpaymainDto prpLpaymaintgDto = blPrpLpaymaintgFacade.findByPrimaryKey(paymentNo);
            ArrayList<PrpLpayShowDto> prpLpayShowtgDtoList = new ArrayList<PrpLpayShowDto>();
    		BLPrpLpayShowtgFacade blPrpLpayShowtgFacade = new BLPrpLpayShowtgFacade();
    		PrpLpayExtDto prpLpayExttgDtoNew = null;
    		if(prpLpaymaintgDto!=null){
    			String claimNoHis = "";
    			String compensateNoHis = "";
    			String payTypeHis = "";
    			String bankTypeName = uiCodeAction.translateCodeCode("BankCode", prpLpaymaintgDto.getBankType(), true);
    			prpLpaymaintgDto.setBankTypeName(bankTypeName);
    			ArrayList<PrplpayDto> collection = (ArrayList<PrplpayDto>)blPrplpaytgFacade.findByConditions(" serialno='"+paymentNo+"'");
    			for(int i=0;i<collection.size();i++){
    				certiNos.add(collection.get(i).getCompensateno());
    				registNos.add(collection.get(i).getRegistno());
    				claimNoHis = collection.get(i).getClaimno();
    			...
    			//TODO 测试
    			prpLpayShowtgDtoList = blPrpLpayShowtgFacade.getShowList(prpLpaymaintgDto.getPayType(), prpLpaymaintgDto.getPaymentType(),prpLpaymaintgDto.getComCode(),paymentNo, true,registNos, certiNos,"4");
    			BLPrpLpayHistgFacade blPrpLpayHistgFacade = new BLPrpLpayHistgFacade();
    			String conditions = " claimNo = '"+claimNoHis+"' and compensateNo = '"+compensateNoHis+"' and payType ='"+payTypeHis+"' order by serialNo2 desc ";
    			ArrayList<PrpLpayHisDto>  prpLpayHistgDtoList = (ArrayList)blPrpLpayHistgFacade.findByConditions(conditions);
    			PrpLpayHisDto prpLpayHistgDto = new PrpLpayHisDto() ;
    			if(prpLpayHistgDtoList.size()>0){
    				prpLpayHistgDto = prpLpayHistgDtoList.get(0);
    			}
    			request.setAttribute("prpLpayHisDto", prpLpayHistgDto);
    			
    			//add by wangxinyang 新农险区分领款人类型 2018-8-8 19:56:20
    			String condition = " systemFlag='agri' and businessno='"+compensateNoHis+"'";
    			...
    			swfLogDtoList = (List<SwfLogDto>)blSwfLogFacade.findByConditions(condition);
    			swfLogStoreDtoList = (List<SwfLogStoreDto>)blSwfLogStoreFacade.findByConditions(condition);
    			if(swfLogDtoList!=null && swfLogDtoList.size()>0 ||
    					swfLogStoreDtoList!=null && swfLogStoreDtoList.size()>0){
    				request.setAttribute("newAgriFlag", "newAgri");
    			}
    			...
    			if("deal".equals(undwrtEditType)){
    				...
    				if("6".equals(prpLpaymaintgDto.getVflag())){
    					//如果在两核系统中的editType是支付审批，则此处支付状态为支付占用
    					new BLPrplpaytgFacade().updateVFlag("2", paymentNo);//更新为正在审核状态
    					new BLPrpLpaymaintgFacade().updateByKey(paymentNo, "2");
    				} 	
    				String paytExtSql = " paymentNo = '"+paymentNo+"' order by inputdate "	;
    				prpLpayExttgDtoList=(ArrayList<PrpLpayExtDto>) blPrpLpayExttgFacade.findByConditions(paytExtSql);
    				for(int i=0;i<prpLpayExttgDtoList.size();i++){
    					String nodeName="";
    					if("apay".equals(prpLpayExttgDtoList.get(i).getNodeType())){
    						nodeName = "新增支付";
    					}else if("vpay".equals(prpLpayExttgDtoList.get(i).getNodeType())){
    						nodeName = "支付审批";
    					}else{
    						nodeName = uiCodeAction.translateCodeCode("ClaimNodeType", prpLpayExttgDtoList.get(i).getNodeType(), true);
    					}
    					String comCName = new UICodeAction().translateComCode(prpLpayExttgDtoList.get(i).getComCode(), true);
    					String userName = new UICodeAction().translateUserCode(prpLpayExttgDtoList.get(i).getOperatorCode(), true);
    					prpLpayExttgDtoList.get(i).setNodeName(nodeName);
    					prpLpayExttgDtoList.get(i).setComCName(comCName);
    					prpLpayExttgDtoList.get(i).setOperatorCodeName(userName);
    				}
    				
    				String starDate = "";
    				//取核赔通过时间，若多个案件，则取最大值
    				if(prpLpayShowtgDtoList!=null&&prpLpayShowtgDtoList.size()>0){
    					if(prpLpayShowtgDtoList.size()==1){
    						starDate=prpLpayShowtgDtoList.get(0).getUnderWriteEndDate();
    					}else{
    						for(int i=1;i<prpLpayShowtgDtoList.size();i++){
    							String bDate = prpLpayShowtgDtoList.get(i).getUnderWriteEndDate();
    							String eDate = prpLpayShowtgDtoList.get(i-1).getUnderWriteEndDate();
    							int temp =DateCompute.compareDate(bDate,eDate);
    							if(temp>=0){
    								starDate=prpLpayShowtgDtoList.get(i).getUnderWriteEndDate();
    							}
    						}
    					}
    				}
    				this.getCostTime(prpLpayExttgDtoList, starDate);
    				
    			}else{
    				String payExtSql = " paymentNo= '"+paymentNo+"'  "	;
    				prpLpayExttgDtoList=(ArrayList<PrpLpayExtDto>) blPrpLpayExttgFacade.findByConditions(payExtSql);
    				for(int i=0;i<prpLpayExttgDtoList.size();i++){
    					String nodeName="";
    					if("apay".equals(prpLpayExttgDtoList.get(i).getNodeType())){
    						nodeName = "新增支付";
    		...
    		request.setAttribute("prpLpayExtDtoNew", prpLpayExttgDtoNew);
    		request.setAttribute("prpLpaymainDto", prpLpaymaintgDto);
    		request.setAttribute("prpLpayShowDtoList", prpLpayShowtgDtoList);
    		request.setAttribute("prpLpayExtDtoList", prpLpayExttgDtoList);
		...
		if(prpLpaymainDto!=null){
			...
			if("deal".equals(undwrtEditType)){
				...
				if("6".equals(prpLpaymainDto.getVflag())){
					//通过打印输出校验是否已经审核通过了，
					PrpLpaymainDto prpLpaymainDtoCheck = blPrpLpaymainFacade.findByPrimaryKey(paymentNo);
//					System.out.println("校验是否为待审核状态：Vflag="+prpLpaymainDtoCheck.getVflag()+"   VerifyFlag="+prpLpaymainDtoCheck.getVerifyFlag()
//							+"   ThirdPayFlag="+prpLpaymainDtoCheck.getThirdPayFlag());
					//如果在两核系统中的editType是支付审批，则此处支付状态为支付占用
//					new BLPrplpayFacade().updateVFlag("2", paymentNo);//更新为正在审核状态
//					new BLPrpLpaymainFacade().updateByKey(paymentNo, "2");
					// 审核占用必须是待审核状态
					new BLPrplpayFacade().updateVFlagFor6("2", paymentNo);//更新为正在审核状态
					new BLPrpLpaymainFacade().updateByKeyFor6(paymentNo, "2");
					//插入支付占用处理意见的操作
...
//					new BLPrpLpayExtFacade().insert(prpLpayExtDto);
				...
				for(int i=0;i<prpLpayExtDtoList.size();i++){
					//String nodeTypeName = "epay".equals(prpLpayExtDtoList.get(i).getNodetype())?"支付":uiCodeAction.translateCodeCode("ClaimNodeType", prpLpayExtDtoList.get(i).getNodetype(), true);
					String nodeName=prpLpayExtDtoList.get(i).getNodeName();
					/*if("apay".equals(prpLpayExtDtoList.get(i).getNodeType())){
						nodeName = "新增支付";
					}else if("vpay".equals(prpLpayExtDtoList.get(i).getNodeType())){
						nodeName = "支付审批";
	...
	public String savePayMain(HttpServletRequest request,String vflag) throws Exception{
		...
		String newAgriFlag = request.getParameter("newAgriFlag");//新农险标识
		...
		String routeNumold = request.getParameter("routeNumold");//原支行行号
		String flag = request.getParameter("Insertflag");//银行信息是否新增标示
		...
		try{
          ...
          //判断是否新增联行号信息
            String prov_code ="";
            String city_code = "";
            String belong_bank_value = "";
            String sql = "select prov_code,city_code,belong_bank_value from prpdbankbranch where prov_name ='"+provinceCode+"'" +
            		" and city_name='"+cityCode+"' and belong_bank_name='"+bankType+"'";
            rs = dbManager.executeQuery(sql);
            if(rs.next()){
            	prov_code = rs.getString("prov_code");
            	city_code = rs.getString("city_code");
            	belong_bank_value = rs.getString("belong_bank_value");
            }
            if("2".equals(flag)){//为新增的支付银行信息
            	String addSql = "insert into prpdbankbranch (bank_number,bank_name,prov_code,prov_name,belong_bank_value,belong_bank_name," +
            			"city_code,city_name,head_bank_flag,detal_bank_code,detal_bank_name,country )" +
            			"values('"+routeNum+"','"+bank+"','"+prov_code+"','"+provinceCode+"','"+belong_bank_value+"','"+bankType+"','"+city_code
            			+"','"+cityCode+"','1','"+prov_code+"','"+provinceCode+"','CHN')";
            	dbManager.executeUpdate(addSql);
            }else if("1".equals(flag)){//为人工已添加的支付银行信息
            	if(!routeNum.equals(routeNumold)){
            		String transSql = "insert into prpdbankbranchhis select * from prpdbankbranch where bank_name='"+bank+"' and head_bank_flag='1'";
            		String updateSql = "update prpdbankbranch set bank_number='"+routeNum+"' where bank_name='"+bank+"' and head_bank_flag='1'";
            		dbManager.executeUpdate(transSql);
		...
		/*保存领款人信息（联合主键：Receiverfullname,Receivertypeother,Certifno,Certiftype,Bank,
		Bankaccount,Banktype,Accountflag,Accounttype,Routenum,City,Province）
		先通过联合主键查询，如果没有则插入一条，如果有1条且电话号码或领款人类型不一样则更新号码和类型。
		*/
		SaveOrDeletePrplpayeeinfoLogger saveOrDeletePrplpayeeinfoLogger = new SaveOrDeletePrplpayeeinfoLogger();
		try{
            PrplPayeeInfoDto prplPayeeInfoDto = new PrplPayeeInfoDto();
			...
			String nowTime = idate.getCurrentTime("yyyy-MM-dd HH:mm:ss");
			prplPayeeInfoDto.setReceiverFullName(receiverFullName);
			prplPayeeInfoDto.setReceiverTypeOther(receiverTypeOther);
			prplPayeeInfoDto.setReceiverTypeOtherName(this.getReceiverTypeOtherName(newAgriFlag, receiverTypeOther));
			prplPayeeInfoDto.setCertifNo(certifNo);
			prplPayeeInfoDto.setCertifType(certifType);
			prplPayeeInfoDto.setBank(bank);
			prplPayeeInfoDto.setBankAccount(bankAccount);
			prplPayeeInfoDto.setBankType(bankType);
			prplPayeeInfoDto.setAccountFlag(accountFlag);
			prplPayeeInfoDto.setAccountType(accountType);
			prplPayeeInfoDto.setRouteNum(routeNum);
			prplPayeeInfoDto.setCity(cityCode);
			prplPayeeInfoDto.setProvince(provinceCode);
			prplPayeeInfoDto.setMobilePhone(mobilePhone);
			prplPayeeInfoDto.setInputDate(nowTime);
			...
			if(!"".equals(cityCode)){
				conditions += " and city = '"+cityCode+"'";
			}else{
				conditions += " and city is null ";
			}
			if(!"".equals(provinceCode)){
				conditions += " and province = '"+provinceCode+"'";
			}else{
			...
			if(!"".equals(conditions)){
				...
				ArrayList list = (ArrayList)bLPrplPayeeInfoFacade.findByConditions(conditions);
				if(list.size()==0){
					blPrplPayeeInfoAction.insert(dbManager,prplPayeeInfoDto);
					dbManager.commitTransaction();
					String data = "新增领款人信息成功："+
							prplPayeeInfoDto.getReceiverFullName()+" "+
							prplPayeeInfoDto.getReceiverTypeOther()+" "+
							prplPayeeInfoDto.getReceiverTypeOtherName()+" "+
							prplPayeeInfoDto.getCertifNo()+" "+
							prplPayeeInfoDto.getCertifType()+" "+
							prplPayeeInfoDto.getBank()+" "+
							prplPayeeInfoDto.getBankAccount()+" "+
							prplPayeeInfoDto.getBankType()+" "+
							prplPayeeInfoDto.getAccountFlag()+" "+
							prplPayeeInfoDto.getAccountType()+" "+
							prplPayeeInfoDto.getRouteNum()+" "+
							prplPayeeInfoDto.getCity()+" "+
							prplPayeeInfoDto.getProvince()+" "+
							prplPayeeInfoDto.getMobilePhone()+" "+
							prplPayeeInfoDto.getInputDate();
					saveOrDeletePrplpayeeinfoLogger.printData(data);
				}
				if(list.size()==1){//更新手机号或领款人类型
					PrplPayeeInfoDto dto = (PrplPayeeInfoDto)list.get(0);
					if(!receiverTypeOther.equals(dto.getReceiverTypeOther())||!mobilePhone.equals(dto.getMobilePhone())){
						String sql = " update prplpayeeinfo set receiverTypeOther = '"+prplPayeeInfoDto.getReceiverTypeOther()+"'," +
								"receiverTypeOtherName='"+prplPayeeInfoDto.getReceiverTypeOtherName()+"',mobilePhone='"+prplPayeeInfoDto.getMobilePhone()+"'," +
								"inputDate='"+nowTime+"' where "+conditions;
						dbManager.executeUpdate(sql);
						dbManager.commitTransaction();
						String data = "更新领款人信息成功："+sql;
						saveOrDeletePrplpayeeinfoLogger.printData(data);
					}
				}
			}
		}catch(Exception exception){
			saveOrDeletePrplpayeeinfoLogger.error(exception);
            dbManager.rollbackTransaction();
        }finally{
            dbManager.close();
            rs.close();
	...
	public void savePays(DBManager dbManager,HttpServletRequest request,String paymentNo,String vflag) throws Exception{
		...
		String [] coinsCodes =  request.getParameterValues("coinsCode");
		...
		for(int i=0;i<registNos.length;i++){
			...
			if("9".equals(vflagHis)){
				...
				String conditions = " Serialno = '"+paymentNoHis+"' and Compensateno = '"+certiNos[i]+"'";
				...
				String conditions1 = "serialno='"+prplpayDto1.getSerialno()+"' and  claimNo ='"+claimNos[i]+"' and Compensateno = '"+certiNos[i]+"' and payType = '"+payType+"'";
			...
			//存储共保人信息
			if(null!=coinsCodes && null!=coinsCodes[i] && !"".equals(coinsCodes[i] )){
				prplpayDto.setCoinsCode(coinsCodes[i]);
			}else{
				prplpayDto.setCoinsCode("");
			}
			list.add(prplpayDto);
		}
...
//				}
	 ...
	 * @throws Exception
	 */
	public void savePrplpayExtBill(DBManager dbManager,HttpServletRequest request,String paymentNo,String vflag) throws Exception{
		String[] rowNum = request.getParameterValues("prpLpayExtRowNum");
		String[] serialNo = request.getParameterValues("prpLpayExtSerialNo");
		String[] InputDate= request.getParameterValues("prpLpayExtInputDate");
		String[] OperatorCode = request.getParameterValues("prpLpayExtOperatorCode");
...
//				}
	...
	public void mergerIssuedPost(HttpServletRequest request,ParamUtils paramUtils)throws Exception{
		...
		DBManager dbManager = new DBManager();
		try{//增加try、catch、finally代码段，保证数据库连接能够正常关闭
	...
	public Map getThirdPayFlagMap() throws Exception {
		...
		thirdPayFlagMap.put("10","同步资金成功");
		thirdPayFlagMap.put("11","重复数据");
		thirdPayFlagMap.put("12","同步资金失败");
		thirdPayFlagMap.put("13","资金支付成功");
		thirdPayFlagMap.put("14","资金支付退票");
		thirdPayFlagMap.put("15","资金退回");
					...
					if(prpLpayBillHisDto!=null){
						...
						if("2".equals(prpLclaimBillManagerDto.getPayFlag())&&!paymentNo.equals(prpLpayBillHisDto.getPaymentNo())){
							if(prpLclaimBillManagerDto!=null){
								...
								prpLpayHisDto.setInterType(prpLpaymainDto.getInterType());
								prpLpayHisDto.setRouteNum(prpLpaymainDto.getRouteNum());
								if(!new PaymentViewHelperNew().checkPayVflag(prplpayDto1.getSerialno()))
							...
							if(!new PaymentViewHelperNew().checkPayVflag(prpLpayBillHisDto.getPaymentNo()))
			...
			if(!"9".equals(vflagHis)&&prpLpayBillHisList.size()>0){
				for(int i=0;i<prpLpayBillHisList.size();i++){
					...
					if(prpLpayBillHisDto!=null&&!paymentNo.equals(prpLpayBillHisDto.getPaymentNo())){
						if(!new PaymentViewHelperNew().checkPayVflag(prpLpayBillHisDto.getPaymentNo()))
							throw new Exception("支付编号"+prpLpayBillHisDto.getPaymentNo()+"为非退回状态不允许操作");
						//多窗口同时支付一个报案号号会造成prplpay重复所以需要删除,2017-08-01是为了防止删除了历史数据
//						blPrplpayAction.deleteByConditions(dbManager, " serialNo = '"+prpLpayBillHisDto.getPaymentNo()+"' and inputdate>=date'2017-08-01' ");
	...
	public String saveBillOrupdate(HttpServletRequest request) throws Exception {
		String submitType = request.getParameter("submitType");
		...
		if(submitType!=null){
			//预赔、理算环节数据收集
			if("0".equals(submitType)){
				return savePayMainBill(request,"");
			}
			//数据暂存
			else if("2".equals(submitType)){
				return savePayMainBill(request,"1");
			}
			//数据提交
			else if("4".equals(submitType)){
				//优化修改
				//billNo = savePayMainBill(request,vflag);
				billNo = savePayMainBillByPrepered(request, vflag);
				//判断案件是否冻结begin
				//DAAReviewViewHelper daaReviewViewHelper = new DAAReviewViewHelper();
				boolean status = false;//daaReviewViewHelper.CaseStatus(null, registNo);
				if(status){
...
//				PrpLpayBillDto.setPayAmount(Double.parseDouble(payAmount[i]);
	...
	public String savePayMainBill(HttpServletRequest request,String vflag) throws Exception{
		...
		try{
    		...
    		if(noBillFlag != null && "1".equals(noBillFlag)){
    			saveBillManagerNoBill(dbManager , request);
    		}
          ...
          DateTime nowTime = new DateTime(DateTime.current().toString(),DateTime.YEAR_TO_SECOND);
            for(int i=0;i<registNo.length;i++){
            	if("0".equals(deleteFlag[i])){
        			...
        			savePaysBill(dbManager,request, paymentNo, vflag,i,prpLregistDto.getComCode());
        			isSave = true;
            ...
            if(isSave){
            	...
            	savePrplpayExtBill(dbManager,request,billNo,vflag);
            }
			dbManager.commitTransaction();
		}catch(Exception exception){
	 ...
	 * @throws Exception
	 */
	private void saveBillManagerNoBill(DBManager dbManager, HttpServletRequest request) throws Exception {
		...
		for(int i=0;i<registNo.length;i++){
			...
			prpLclaimBillManagerDto.setRegistNo(registNo[i]);
			prpLclaimBillManagerDto.setPolicyNo(policyNo[i]);
			prpLclaimBillManagerDto.setCompensateNo(compensateNos[i]);
			prpLclaimBillManagerDto.setSerialNo(Integer.parseInt(fkSerialNo[i]));
			prpLclaimBillManagerDto.setPayName(receiverFullName[i]);
			prpLclaimBillManagerDto.setIdentifyNumber(certifNo[i]);
			prpLclaimBillManagerDto.setInsuredCode(receiverFullCode[i]);
			prpLclaimBillManagerDto.setOpenBank("");
			prpLclaimBillManagerDto.setBankAccount("");
			prpLclaimBillManagerDto.setSumPaid(Double.parseDouble(paids[i]));
			prpLclaimBillManagerDto.setPayAmount(Double.parseDouble(paids[i]));
			if(Double.parseDouble(paids[i])==0){
			...
			if("EDIT".equals(editType)){
				conditions = " registno = '"+registNo[i]+"' and policyno = '"+policyNo[i]+"' and compensateno = '"+compensateNos[i]+"' and serialno = "+Integer.parseInt(fkSerialNo[i])+"" ; 
	 ...
	 * @throws Exception
	 */
	public void savePaysBill(DBManager dbManager,HttpServletRequest request,String paymentNo,String vflag,int index,String comCode) throws Exception{
		UserDto user = (UserDto)request.getSession().getAttribute("user");
		String editType = request.getParameter("editType");
		String payType = request.getParameter("payType");//支付类型
		String Datatype = request.getParameter("Datatype");//合并支付标志
		String paymentType = request.getParameter("paymentType");//赔款类型
		String receiverTypeOther = request.getParameterValues("receiverTypeOther")[index];//领款人类型代码
		String receiverTypeOtherName = request.getParameterValues("receiverTypeOtherName")[index];//领款人类型名称
		String receiverFullName = request.getParameterValues("payName")[index];//领款人名称
		...
		String  payrefreasons = request.getParameter("paymentType");
		...
		for(int i=0;i<compensateNo.length;i++){
			prplpayDto = new PrplpayDto();
			prplpayDto.setClaimno(claimNo);
			prplpayDto.setSerialno(paymentNo);
			prplpayDto.setSerialNo2(String.valueOf(i+1));
			prplpayDto.setCompensateno(compensateNo[i]);
			prplpayDto.setClasscode(policyNo.substring(1, 3));
			prplpayDto.setRiskcode(policyNo.substring(1,5));
			prplpayDto.setRegistno(registNo);
			prplpayDto.setPolicyno(policyNo);
			prplpayDto.setPaytype(payType);
			
			prplpayDto.setReceivertype(receiverTypeOther);
			double payAmount1 = Double.parseDouble(payAmount[i]);
	 ...
	 * @throws Exception
	 */
	public String savePayMainBillByPrepered(HttpServletRequest request,String vflag) throws Exception{
		String editType = request.getParameter("editType");
		String payType = request.getParameter("payType");//支付类型
		String Datatype = request.getParameter("Datatype");//合并支付标志
		String[] receiverTypeOther = request.getParameterValues("receiverTypeOther");//领款人类型代码
		...
		try{
            ...
            if("ADD".equals(editType)&&billFlag!=null&&"1".equals(billFlag)){
				...
				//优化修改
				//riskCode = this.getRiskCodeByRegistNo(registNo[0], dbManager);
				riskCode = this.getRiskCodeByRegistNoByPrepered(registNo[0], dbManager);
				comCode = user.getComCode();
				year = new DateTime(DateTime.current().toString()).getYear();
				//优化修改
				//billNo = uiBillAction.getNo(dbManager,tableName, riskCode,comCode, year,"");
            ...
            for(int i=0;i<registNo.length;i++){
            	if("0".equals(deleteFlag[i])){
                	...
                	if(billFlag!=null&&"1".equals(billFlag)){
        				...
        				//优化修改
        				//savePayBillLink(dbManager,request,paymentNo,billNo,i,user.getUserCode());//??????????????????
        				savePayBillLinkByprepered(dbManager,request,paymentNo,billNo,i,user.getUserCode());
	...
	public void savePayBillLinkByprepered(DBManager dbManager,HttpServletRequest request,String paymentNo,String billNo,int index,String userCode) throws Exception{
		String submitType = request.getParameter("submitType");
		String certiNos = request.getParameterValues("compensateNo")[index];
		String registNos = request.getParameterValues("registNo")[index];
		String paids = request.getParameterValues("payAmount")[index];
...
//				prpLpayBillHisDto = blPrpLpayBillAction.findByPrimaryKey(dbManager, prpLpayBillDto.getRegistNo(), prpLpayBillDto.getCompensateNo(), prpLpayBillDto.getSerialNo());
	 ...
	 * @throws Exception
	 */
	public Map getRealPayWayMap() throws Exception {
		Map<String, String> realPayWayMap = new HashMap<String,String>();
		String sql = "Select codecode,codecname from  prpdcode Where codetype='RealPayWay'";
		DBManager dbManager = new DBManager();
		try{
			dbManager.open(AppConfig.get("sysconst.DBJNDI"));
            	ResultSet resultSet = dbManager.executeQuery(sql);
            	while(resultSet.next()){
            		realPayWayMap.put(resultSet.getString("codecode"), resultSet.getString("codecname"));       
            	}
		}catch(Exception exception){
			exception.printStackTrace();
            dbManager.rollbackTransaction();
	 ...
	 * @throws Exception
	 */
	public String getCertifTypeName(String certifType) throws Exception {
		String certifTypeName = "";
		Map<String, String> certifTypeMap = new HashMap<String,String>();
		String sql = "Select codecode,codecname from  prpdcode Where codetype='CertiType'";
		DBManager dbManager = new DBManager();
		try{
			dbManager.open(AppConfig.get("sysconst.DBJNDI"));
            	ResultSet resultSet = dbManager.executeQuery(sql);
            	while(resultSet.next()){
            		certifTypeMap.put(resultSet.getString("codecode"), resultSet.getString("codecname"));       
            	}
		}catch(Exception exception){
			exception.printStackTrace();
            dbManager.rollbackTransaction();
		...
		certifTypeName = certifTypeMap.get(certifType);
	 ...
	 * @throws Exception
	 */
	public String getReceiverTypeOtherName(String flag,String receiverTypeOther) throws Exception {
		String receiverFullName = "";
		Map<String, String> receiverTypeOtherMap = new HashMap<String,String>();
		if(flag!=null && "newAgri".equals(flag)){
			String sql = "Select codecode,codecname from  prpdcode Where codetype='ReceiverType'";
			DBManager dbManager = new DBManager();
			try{
				dbManager.open(AppConfig.get("sysconst.DBJNDI"));
	            	ResultSet resultSet = dbManager.executeQuery(sql);
	            	while(resultSet.next()){
	            		receiverTypeOtherMap.put(resultSet.getString("codecode"), resultSet.getString("codecname"));       
	            	}
			}catch(Exception exception){
				exception.printStackTrace();
	            dbManager.rollbackTransaction();
			...
			receiverTypeOtherMap.put("t01", "被保险人");
			receiverTypeOtherMap.put("t02", "查勘直赔中心");
			receiverTypeOtherMap.put("t03", "玻璃更换单位");
			receiverTypeOtherMap.put("t04", "查勘人员");
			receiverTypeOtherMap.put("t05", "人民法院");
			receiverTypeOtherMap.put("t06", "公估机构");
			receiverTypeOtherMap.put("t07", "配件维修单位");
			receiverTypeOtherMap.put("t08", "调查机构");
			receiverTypeOtherMap.put("t09", "救援机构");
			receiverTypeOtherMap.put("t10", "鉴定机构");
			receiverTypeOtherMap.put("t11", "律师部门");
			receiverTypeOtherMap.put("t12", "其他个人");
			receiverTypeOtherMap.put("t13", "其他单位");
			receiverTypeOtherMap.put("t14", "受害人员");
			receiverTypeOtherMap.put("t15", "救治医院");
			receiverTypeOtherMap.put("t16", "民政部门");
			receiverTypeOtherMap.put("t17", "交警部门");
			receiverTypeOtherMap.put("t18", "仲裁机构");
			receiverTypeOtherMap.put("t20", "委托代理人");
		}
		receiverFullName = receiverTypeOtherMap.get(receiverTypeOther);

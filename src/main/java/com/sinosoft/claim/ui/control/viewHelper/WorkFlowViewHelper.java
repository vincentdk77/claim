...
import com.gyic.claim.bl.facade.BLSwfLogFacadeBase;
...
import com.sinosoft.claim.ui.control.viewHelper.WorkFlowImageViewHelper;
...
//	        }
	...
	private String getNodeTaskListConditions(
		...
		//添加区分新农险标识
		//condition += " and (systemflag is null or systemflag<>'agri') ";
		...
		if (status.equals("0")) {
			...
			if (nodeType.equals("sched")) { // 定损调度太特殊了
				...
				condition = " (nodeType='" + nodeType + "' ) and (NodeStatus='"
						+ status + "') and flowStatus!='0' and (handlerCode='" + handlerCode
						+ "' or handlerCode is null or handlerCode='')";
				//非农险由总公司集中报案，各中支公司只能受理农险报案(意键险组合保险也可以了哦)  理赔部要求农房农房机构报案20180713
				if(user.getComCode().indexOf("000") != 0){
					condition += " and  (riskcode like '31%' or riskcode like '32%' or riskcode like 'ZH%' or riskcode like '27%' or riskcode like '26%' or riskcode like '28%' or riskcode like '0310') ";
				}
...
//            }
		...
		if("3".equals(status)){
			if (statEndDate != null && statEndDate.trim().length() > 0) {
				condition = condition //handletime FlowInTime
				        + "And "+"to_date(flowintime,'YYYY-MM-DD HH24:mi:ss') <="
		...
		if(strHPflag != null && !strHPflag.equals("") && !strHPflag.equals("null")) {
			if(strHPflag.equals("Large")) {
				condition = condition
				+ " and policyno in (select policyno from "
				+ "prpcmain where policytype in('H24', 'I28', 'E2', 'Q2')" +
						")";
				System.err.println("condition-11----"+condition);
			}
			if(strHPflag.equals("Retaol")) {
				condition = condition
				+ " and policyno in (select policyno from "
				+ "prpcmain where policytype in('H23', 'I27', 'E1', 'Q1')" +
						")";
				System.err.println("condition--11---"+condition);
			}	
		}
...
//            }
	...
	public WorkFlowDto changeFlowNodeHandler(String flowID, String logNo,
		...
		if("agri".equals(swfLogDto.getSystemFlag())){//新农险改派数据
			swfLogDto.setSystemFlag("agri");
		}

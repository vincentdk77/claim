		...
		if (prpLpropSerialNo==null){
			...
			for(int index=1;index<prpLpropSerialNo.length;index++)
			{
				...
				if(prpLpropRiskCode!=null){
					if(prpLpropRiskCode.indexOf("05")>-1){
						prpLpropDto.setLossItemCode             ("S");
					}else{
...
////			System.out.println("-------------对象----2--------"+certainLossDto.getPrpLverifyLossDto());
	...
	public void setPrpLcertainLossDtoToView(HttpServletRequest httpServletRequest,WorkFlowQueryDto workFlowQueryDto,int pageNo,int recordPerPage) throws Exception
	{
		...
		//添加区分新农险标识
		conditions = conditions + " and (b.BusinessNo in (select registno from swflog where (systemflag is null or systemflag <> 'agri') and nodetype='"+nodeTypeTemp+"') ";
		conditions = conditions + " or b.BusinessNo in (select registno from swflogstore where (systemflag is null or systemflag <> 'agri') and nodetype='"+nodeTypeTemp+"')) ";
...
//		conditions = conditions + StringConvert.convertString("a.policyNo",policyNo,workFlowQueryDto.getPolicyNoSign());
	 ...
	 * @throws Exception
	 */
	public void setUnitCollection(HttpServletRequest httpServletRequest, String riskCode)
			throws Exception {

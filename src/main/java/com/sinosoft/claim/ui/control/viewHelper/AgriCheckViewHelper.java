...
public class AgriCheckViewHelper extends CheckViewHelper {
	/**
	 * 默认构造方法
	 */
	public AgriCheckViewHelper() {
	}
...
//			增加财产损失定损调度信息，如果有人，就进行调度
	...
	public void checkDtoView(HttpServletRequest httpServletRequest,
		...
		//养殖险已转交查勘，prplcheckdto为null时，进行处理。
		if(null==prpLcheckDto){
			...
			//查询相同保单号的出险次数
			agriRegistViewHelper.getSamePolicyRegistInfo(httpServletRequest,
			...
			//向页面放置理赔清单号
			String conditions=" reportcode='"+checkNo+"' and validity='2'";
			...
			Collection blSettlemainlist=blSettlemainlistFacade.findByConditions(conditions);
			Collection blSettlemainlistForMedical=blSettlemainlistFacadeForMedical.findByConditions(conditions);
			...
			agriCertainLossViewHelper.registDtoToView(httpServletRequest, checkNo,
					editType);
					/*BusinessRuleUtil.getOuterCode(httpServletRequest,
	...
	private void setSubInfo(HttpServletRequest httpServletRequest,
		...
		if(checkDto.getPrpLregistTextDtoList3() != null){
			httpServletRequest.setAttribute("prpLregistTextDtoList3", checkDto.getPrpLregistTextDtoList3());
		}
	...
	public void verifyLossDtoToView(HttpServletRequest httpServletRequest,
		...
		verifyLossDto = bLVerifyLossFacade.findByPrimaryKey(registNo, null,
				null);
		httpServletRequest.setAttribute("prpLpropDtoList", verifyLossDto
				.getPrpLpropDtoList());
	}

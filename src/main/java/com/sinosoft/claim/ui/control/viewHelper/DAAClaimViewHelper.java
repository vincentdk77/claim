...
public class DAAClaimViewHelper extends ClaimViewHelper {
	...
	private static Log logger = LogFactory.getLog(DAAClaimViewHelper.class);
    /**
     * 默认构造方法
     */
    ...
    public ClaimDto viewToDto(HttpServletRequest httpServletRequest) throws Exception {
        ...
        //乡村干部组合保险赔付时需到人或户
    	String[] prpLacciPersonFamilyNo = httpServletRequest.getParameterValues("prpLclaimLossFamilyNo");
        String[] prpLacciPersonAcciName = httpServletRequest.getParameterValues("prpLclaimLossFamilyName");
        
        //自行车第三者责任保险赔付时需到车辆
    	String[] prpLclaimLossFamilyNo = httpServletRequest.getParameterValues("prpLclaimLossFamilyNo");
        String[] prpLclaimLossBrandCode = httpServletRequest.getParameterValues("prpLclaimLossBrandCode");
        String[] prpLclaimLossCarNumber = httpServletRequest.getParameterValues("prpLclaimLossCarNumber");
        //0313碎屏险添加IMEI码 add by wangxinyang 2018/6/19
        String[] prpLclaimLossImei = httpServletRequest.getParameterValues("prpLclaimLossImei");
        ...
        if (prpLclaimLossCurrency == null) {
            ...
            for (int index = 1; index < prpLclaimLossCurrency.length; index++) {
               ...
               ////非车没有上报估损这一项------------------------------------------------
                ...
                if("ZH01".equals(prpLclaimLossRiskCode) || "ZH02".equals(prpLclaimLossRiskCode )){
                	if("".equals(prpLacciPersonFamilyNo[index]) || prpLacciPersonFamilyNo[index] == null ){
                		prpLclaimLossDto.setFamilyNo(0);
                	}else{
                		prpLclaimLossDto.setFamilyNo(Integer.parseInt(prpLacciPersonFamilyNo[index]));
                	}
                	prpLclaimLossDto.setFamilyName(prpLacciPersonAcciName[index]);
                }
                if( "ZH03".equals(prpLclaimLossRiskCode)){
                		prpLclaimLossDto.setFamilyNoZH(prpLacciPersonFamilyNo[index]);
                		prpLclaimLossDto.setFamilyName(prpLacciPersonAcciName[index]);
                }
                if("1528".equals(prpLclaimLossRiskCode) ){
                	if("".equals(prpLclaimLossFamilyNo[index]) || prpLclaimLossFamilyNo[index] == null ){
                		prpLclaimLossDto.setFamilyNo(0);
                	}else{
                		prpLclaimLossDto.setFamilyNo(Integer.parseInt(prpLclaimLossFamilyNo[index]));
                	}
                    if(prpLclaimLossBrandCode!=null&&prpLclaimLossBrandCode.length>index){
                	   prpLclaimLossDto.setItemDetailName(prpLclaimLossBrandCode[index]);
                    }
                    if(prpLclaimLossCarNumber!=null&&prpLclaimLossCarNumber.length>index){
                	   prpLclaimLossDto.setLicenseNo(prpLclaimLossCarNumber[index]);
                    }
                }
                if("0313".equals(prpLclaimLossRiskCode)){
                	if(prpLclaimLossImei!=null && prpLclaimLossImei[index]!=null && !"".equals(prpLclaimLossImei[index])){
                		prpLclaimLossDto.setImei(prpLclaimLossImei[index]);
                	}
                }
    ...
    public PrpLclaimDto registDtoToView(HttpServletRequest httpServletRequest, String registNo) throws Exception {
        ...
        if ( prpLcheckDto==null){
         ...
         {
        	 prpLclaimDto.setRemark(prpLcheckDto.getRemark());//
         }
...
//        prpLclaimDto.setLossesUnitCode(prpLregistDto.getLossesUnitCode());
        ...
        if ("H".equals(strRiskType) || "Q".equals(strRiskType) || "I".equals(strRiskType)
			...
			while (it.hasNext()) {
				...
				if ("ZH03".equals(prpLpropDto.getRiskCode())) {
					prpLclaimLossDto.setSumClaim(prpLpropDto.getSumDefLoss()); // 估损金额
				}
				...
				if("0310".equals(prpLpropDto.getRiskCode())||"0312".equals(prpLpropDto.getRiskCode())||riskcodeStr.indexOf(riskCode)>0 )
				{
					double claimsumloss = 0;
					//double verifyloss = prpLpropDto.getVeriSumDefLoss();
					if(prpLpropDto.getVeriSumDefLoss() > 0)
					{
						prpLclaimLossDto.setSumClaim(prpLpropDto.getVeriSumDefLoss());
					}
					else if(prpLpropDto.getVeriSumLoss() > 0)
					{
						prpLclaimLossDto.setSumClaim(prpLpropDto.getVeriSumLoss());
					}
					else
					{
				...
				prpLclaimLossDto.setInputDate(new DateTime(new Date()));
				if ("1528".equals(prpLpropDto.getRiskCode())) {
					prpLclaimLossDto.setItemDetailName(prpLpropDto.getModelNo());
					prpLclaimLossDto.setLicenseNo(prpLpropDto.getLicenseNo());
					prpLclaimLossDto.setFamilyNo(prpLpropDto.getFamilyNo());
				}
				if("0313".equals(prpLpropDto.getRiskCode())){
					if(prpLpropDto.getImei()!=null && !"".equals(prpLpropDto.getImei())){
						prpLclaimLossDto.setImei(prpLpropDto.getImei());
					}
				}
    ...
    public void setPrpLclaimDtoToView(HttpServletRequest httpServletRequest,
		...
		//添加区分新农险标识
		conditions = conditions +" and (prplclaim.registNo in (select registno from swflog where (systemflag is null or systemflag <> 'agri') and nodetype='claim') ";
		conditions = conditions +" or prplclaim.registNo in (select registno from swflogstore where (systemflag is null or systemflag <> 'agri') and nodetype='claim')) ";
		...
		conditions = conditions
		+ StringConvert.convertString("prplclaim.riskcode",
				queryRiskCode, "=");
        ...
        if(strHPflag != null && !strHPflag.equals("") && !strHPflag.equals("null")) {
      	  if(strHPflag.equals("Large")) {
      		  conditions = conditions +  " and prplclaim.policyno in (select policyno from prpcmain where policytype in('H24', 'I28', 'E2', 'Q2'))";
      	  }
            if(strHPflag.equals("Retaol")) {
          	  conditions = conditions +  " and prplclaim.policyno in (select policyno from prpcmain where policytype in('H23', 'I27', 'E1', 'Q1'))";
      	  }
        }
...
||prpCitemKindDto.getKindCode().equalsIgnoreCase("BZ")
 ...
 //System.out.println("aaaaaaaaaaaa"+prpLclaimDto.getLanguage());       
    ...
    public ClaimDto cancelViewToDto(HttpServletRequest httpServletRequest) throws Exception {
        ...
        String prpCancel = httpServletRequest.getParameter("prpCancel");
        String prpExclusions = httpServletRequest.getParameter("prpExclusions");
        /*---------------------立案主表prpLclaim------------------------------------*/
    ...
    public void cancelDtoToCancelView(HttpServletRequest httpServletRequest, UserDto user) throws Exception {
        ...
        if ((claimNo == null || claimNo.equals(""))) {
            ...
            if("claim".equals(type)){
            	strSql += " and canceldate is not null";
            }
...
//              BZ不带入到估损里面
   ...
   // System.out.println("^^^^^^^^^^^^^^^^^KindCode^^^^^^^^^^^^^^^^^^^"+prpLclaimLossDto.getKindCode());
    ...
    //双核调用方法，初始化立案注销。
    public void nucancelDtoToView(HttpServletRequest httpServletRequest)throws Exception{
        ...
        ArrayList claimList = new ArrayList();
		UIClaimAction uiClaimAction = new UIClaimAction();
        String  strSql = "registNo='" + prpLclaimDto.getRegistNo() + "' and endcasedate is null  ";
        claimList = (ArrayList)uiClaimAction.findByConditions(strSql);
	  	httpServletRequest.setAttribute("prpLclaimDtoList", claimList);
		httpServletRequest.setAttribute("prpLclaimDto", prpLclaimDto);
        PrpLRegistRPolicyDto  prpLregistRPolicyDto = new BLPrpLRegistRPolicyFacade().findByPrimaryKey(prpLclaimDto.getRegistNo(), prpLclaimDto.getPolicyNo());
        httpServletRequest.setAttribute("prpLregistRPolicyDto", prpLregistRPolicyDto);
    }

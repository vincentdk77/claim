...
function saveFormNew(field,saveType)
 { 
	...
	// 3204养鸡保险需要增加是否输入定损清单判断
	var riskCode = fm.riskcode.value;
	if(riskCode == '3204')
	{
	if(document.all("Prop").tBodies.item(0).rows.length > 0)
	{
			if(fm.prpLpropFamilyName[1]==null ||fm.prpLpropFamilyName[1].value=="")
			{
				alert("清单内容不能为空！");
				return false;
			}
	} else {
				alert("3204险种必需填写核定损清单！");
				return false;
			}
	}
	
	return saveForm(field,saveType);
}
</script>
<%-- 保存通用按钮 --%>              

...
<script language="javascript">
  ...
  function deleteItems(){
  	...
  	if("PolicyList"==dataSource){
  		...
  		if(nodeType==realNodeType){
			...
			if(checked){
		  		fm.action = "/claim/deletePlantingSettleList.do"
		  		fm.submit();
	  		}else{
	  			alert("请至少选择一个农户理赔信息！");
	  			return false;
  ...
  function importSettle(){
	...
	if (!res)
		...
		document.fm.action="/claim/plantingSettleImport.do?";
 ...
 function save(){
 	...
 	if("loadXLS"!=loadXLS){
		...
		initScroll('scrlContainer', 'scrlContent');
 		document.fm.action="/claim/plantingSettleSave.do?";
 		document.fm.submit();
 	}
 }
...
<%	
	...
	List<String> strTitle = UIClaimGetSettleListFacade.getTitleListByRisk(riskCode); //存放显示页面清单表头信息
	List<String> strTitleFieldName = UIClaimGetSettleListFacade.getTitleFieldNameListByRiskPolicyNo(riskCode,policyNo);//存放列表中字段对应的数据库字段名，没有的字段字符型使用STRING，数字型用INT中
	
	UIClaimGetSettleListFacade uiClaimGetSettleListFacade = new UIClaimGetSettleListFacade();
	List settleInfoList = uiClaimGetSettleListFacade.getSettleInfoList(request, loadXLS,  editType, strTitleFieldName);
%>
  <body onLoad="initPage()" >
     <form name="fm" action="/claim/settleList.do" method="post" enctype="multipart/form-data">
<table class="common">
...
%>
	<table id="table1" width="100%" class="common">
 	<tr align=middle>
	    ...
	    <%if(null != strTitle && strTitle.size()>0 ){
	    	for(int titleIndex =0 ;titleIndex < strTitle.size(); titleIndex ++){
		    	if("ALLSELECT".equals(strTitleFieldName.get(titleIndex))){%>
			    	
		    	<%}else {%>
		    		<td class="centertitle"><%=strTitle.get(titleIndex) %></td>
		   	 	<%}%>
	     	<%}%>
	    <%}%>
     </tr>
     ...
     <% 
     	if(null!=settleInfoList && settleInfoList.size()>0){
     		Iterator it = settleInfoList.iterator();
     		int i=1;
     		while(it.hasNext()){
     			List<String> settleRowInfoList = (List<String>)it.next();
     			if(null != settleRowInfoList && settleRowInfoList.size() >0){
			     %>
			     <tr class="listeven">
			     	<td>
						<input type="checkbox" name="settleIndexes" id="settleIndexes" value="<%=settleRowInfoList.get(0)%>" onclick="cbOnclick(this)"/>
						<input type="hidden" name="settleIndex<%=i%>" value="<%=settleRowInfoList.get(0)%>"/>
					</td>
				<% 
				for(int indexOfField = 0;indexOfField<settleRowInfoList.size();indexOfField++){
					if("ALLSELECT".equals(strTitle.get(indexOfField))){%>
				<%}else if("INDEXOFSETTLE".equals(strTitle.get(indexOfField))){ %>
					<td>
						<%=i %>
					</td>
				<%}else if("INSUREAREA".equals(strTitle.get(indexOfField))){
					%>
					<td id="insuredArea<%=i %>">
						<%=settleRowInfoList.get(indexOfField)%>
					</td>
				<%}else if ("SUMINSURED".equals(strTitle.get(indexOfField))){
					%>
					<td id="sumInsured<%=i %>">
						<%=settleRowInfoList.get(indexOfField)%>
					</td>
				<%}else if ("KINDCODE".equals(strTitle.get(indexOfField))){
					%>
					<td id="kindCode<%=i %>">
						<%=settleRowInfoList.get(indexOfField)%>
					</td>
				<%}else if ("FIELDSOURCE".equals(strTitle.get(indexOfField))){
					%>
					<td id="fieldSourceName<%=i %>">
						<%=settleRowInfoList.get(indexOfField)%>
					</td>
				<%}else if ("CLAIMRATE".equals(strTitle.get(indexOfField))){
					%>
					<td id="claimRate<%=i %>">
						<%=settleRowInfoList.get(indexOfField)%>
					</td>
				<%}else if ("SETTLEAREA".equals(strTitle.get(indexOfField))){
					%>
					<td id="settlearea<%=i %>">
						<%=settleRowInfoList.get(indexOfField)%>
					</td>
				<%}else if ("LOSSRATE".equals(strTitle.get(indexOfField))){
					%>
					<td id="lossrate<%=i %>">
						<%=settleRowInfoList.get(indexOfField)%>
					</td>
				<%}else if ("SETTLESUM".equals(strTitle.get(indexOfField))){
					%>
					<td id="settlesum<%=i %>">
						<%=settleRowInfoList.get(indexOfField)%>
					</td>
				<%}else if ("REMARK".equals(strTitle.get(indexOfField))){
					%>
					<td id="remark<%=i %>">
						<%=settleRowInfoList.get(indexOfField)%>
					</td>
				<%}else if ("TAXAREA".equals(strTitle.get(indexOfField))){
					%>
					<td>
						<%=settleRowInfoList.get(indexOfField)%>
					</td>
				<%}else if ("ATAREA".equals(strTitle.get(indexOfField))){
					%>
					<td>
						<%=settleRowInfoList.get(indexOfField)%>
					</td>
				<%}else if ("WOODLANDAREA".equals(strTitle.get(indexOfField))){
					%>
					<td>
						<%=settleRowInfoList.get(indexOfField)%>
					</td>	
				<%}else if ("LITTERAREA".equals(strTitle.get(indexOfField))){
					%>
					<td>
						<%=settleRowInfoList.get(indexOfField)%>
					</td>
...
<table id="table1" width="100%" class="common">
     ...
     <tr class="listeven">
		<td>
		<input type="checkbox" name="settleIndexes" id="settleIndexes" value="<%=plantingsettlelistDto.getIndexofsettle()%>" onclick="cbOnclick(this)"/>
		<input type="hidden" name="settleIndex<%=i%>" value="<%=plantingsettlelistDto.getIndexofsettle()%>"/>
		</td>
		<td>
		<%=i %>
		</td>
		<td>
		<%=plantingsettlelistDto.getFcode()==null?"&nbsp;":plantingsettlelistDto.getFcode() %>
		</td>
		<td>
		<%=plantingsettlelistDto.getFname()==null?"&nbsp;":plantingsettlelistDto.getFname()%>
		</td>
		<td>
		<%=plantingsettlelistDto.getFidcard()==null?"&nbsp;":plantingsettlelistDto.getFidcard()%>
		<td>
		<%=plantingsettlelistDto.getZhibuka()==null?"&nbsp;":plantingsettlelistDto.getZhibuka()%>
		</td>
		<td id="insuredArea<%=i %>">
		<%=plantingsettlelistDto.getInsurearea()==0?0:plantingsettlelistDto.getInsurearea()%>
		</td>
		<td id="sumInsured<%=i %>">
		<%=plantingsettlelistDto.getSuminsured()==null?0:plantingsettlelistDto.getSuminsured()%>
		</td>
		</td>
		<td id="kindCode<%=i %>">
		<%=plantingsettlelistDto.getKindcode()==null?"&nbsp;":plantingsettlelistDto.getKindcode()%>
		</td>
		<td id="fieldSourceName<%=i %>">
		<% 
		...
		<td id="claimRate<%=i %>">
		<%=plantingsettlelistDto.getClaimrate()==0?0:plantingsettlelistDto.getClaimrate()%>
		</td>
		<!-- td id="growingseason<%//=i %>">
		<%//=plantingsettlelistDto.getGrowingseason()==0?0:plantingsettlelistDto.getGrowingseason()%>
		</td> -->
		<td id="settlearea<%=i %>">
		<%=plantingsettlelistDto.getSettlearea()==0?0:plantingsettlelistDto.getSettlearea()%>
		</td>
		<td id="lossrate<%=i %>">
		<%=plantingsettlelistDto.getLossrate()==0?0:plantingsettlelistDto.getLossrate()%>
		</td>
		<td id="settlesum<%=i %>">
		<%=plantingsettlelistDto.getSettlesum()==null? "0":plantingsettlelistDto.getSettlesum()%>
		</td>
		<td id="remark<%=i %>">
		<%=plantingsettlelistDto.getRemark()==null?"&nbsp;":plantingsettlelistDto.getRemark()%>
		</td>
	</tr>
	<%		i++;
			}
     ...
     <tr class="listeven">
		<td>
		<input type="checkbox" name="settleIndexes" id="<%=i%>" value="<%=plantingPolicyListDto.getIndexcode()%>" onclick="cbOnclick(this)"/>
		<input type="hidden" name="settleIndex<%=i%>" value="<%=plantingPolicyListDto.getIndexcode()%>"/>
		</td>
		<td>
		<%=i %><input type="hidden" name="hiddenInfo" id="hiddenInfo" value />
		</td>
		<td>
		<%=plantingPolicyListDto.getFcode()==null?"&nbsp;":plantingPolicyListDto.getFcode() %>
		</td>
		<td>
		<%=plantingPolicyListDto.getFname()==null?"&nbsp;":plantingPolicyListDto.getFname()%>
		</td>
		<td>
		<%=plantingPolicyListDto.getFidcard()==null?"&nbsp;":plantingPolicyListDto.getFidcard()%>
		<td>
		<%=plantingPolicyListDto.getZhibuka()==null?"&nbsp;":plantingPolicyListDto.getZhibuka()%>
		</td>
		<td id="insuredArea<%=i %>">
		<%=plantingPolicyListDto.getInsurearea()==0?"0":plantingPolicyListDto.getInsurearea()%>
		</td>
		<td id="sumInsured<%=i %>">
		<%=plantingPolicyListDto.getSumamount()==0?"0":plantingPolicyListDto.getSumamount()%>
		</td>
		<td id="kindCode<%=i %>">
		<%=plantingPolicyListDto.getKindcode()==null?"&nbsp;":plantingPolicyListDto.getKindcode()%>
		</td>
		<td id="fieldSourceName<%=i %>">
		<%
		...
		<td id="remark<%=i %>">
		<%=plantingPolicyListDto.getRemark()==null?"&nbsp;":plantingPolicyListDto.getRemark()%>
		</td>
	</tr>
	<%		i++;
			}
     ...
     <tr class="listeven">
		<td>
		<input type="checkbox" name="settleIndexes" id="settleIndexes" value="<%=plantingsettlelistTempDto.getIndexofsettle()%>" onclick="cbOnclick(this)"/>
		<input type="hidden" name="settleIndex<%=i%>" value="<%=plantingsettlelistTempDto.getIndexofsettle()%>"/>
		</td>
		<td>
		<%=i %>
		</td>
		<td>
		<%=plantingsettlelistTempDto.getFcode()==null?"&nbsp;":plantingsettlelistTempDto.getFcode() %>
		</td>
		<td>
		<%=plantingsettlelistTempDto.getFname()==null?"&nbsp;":plantingsettlelistTempDto.getFname()%>
		</td>
		<td>
		<%=plantingsettlelistTempDto.getFidcard()==null?"&nbsp;":plantingsettlelistTempDto.getFidcard()%>
		<td>
		<%=plantingsettlelistTempDto.getZhibuka()==null?"&nbsp;":plantingsettlelistTempDto.getZhibuka()%>
		</td>
		<td id="insuredArea<%=i %>">
		<%=plantingsettlelistTempDto.getInsurearea()==0?0:plantingsettlelistTempDto.getInsurearea()%>
		</td>
		<td id="sumInsured<%=i %>">
		<%=plantingsettlelistTempDto.getSuminsured()=="0"?0:plantingsettlelistTempDto.getSuminsured()%>
		</td>
		</td>
		<td id="kindCode<%=i %>">
		<%=plantingsettlelistTempDto.getKindcode()==null?"&nbsp;":plantingsettlelistTempDto.getKindcode()%>
		</td>
		<td id="fieldSourceName<%=i %>">
		<% 
		...
		<td id="claimRate<%=i %>">
		<%=plantingsettlelistTempDto.getClaimrate()==0?0:plantingsettlelistTempDto.getClaimrate()%>
		</td>
		<!-- td id="growingseason<%//=i %>">
		<%//=plantingsettlelistDto.getGrowingseason()==0?0:plantingsettlelistDto.getGrowingseason()%>
		</td> -->
		<td id="settlearea<%=i %>">
		<%=plantingsettlelistTempDto.getSettlearea()==0?0:plantingsettlelistTempDto.getSettlearea()%>
		</td>
		<td id="lossrate<%=i %>">
		<%=plantingsettlelistTempDto.getLossrate()==0?0:plantingsettlelistTempDto.getLossrate()%>
		</td>
		<td id="settlesum<%=i %>">
		<%=plantingsettlelistTempDto.getSettlesum()==null?"0":plantingsettlelistTempDto.getSettlesum()%>
		</td>
		<td id="remark<%=i %>">
		<%=plantingsettlelistTempDto.getRemark()==null?"&nbsp;":plantingsettlelistTempDto.getRemark()%>
		</td>
	</tr>
	<%		i++;
			}
...
%>
   ...
   <tr align="center" id="buttonTr">
    ...
    <td>
		<input type="button" value="导出理赔清单模板" name="exportTemplate" class="bigbutton" onclick="window.target='_blank';window.location='plantingSettleExport.do?settleListCode=<%=settleListCode%>&&listType=<%=listType%>&&nodeType=<%=nodeType%>&&insureListCode=<%=insureListCode%>&&riskCode=<%=riskCode%>&&areaCode=<%=areaCode%>&&template=template&&policyNo=<%=policyNo%>'; " style="display:none">
	</td>
	<td>
		<input type="button" value="导出理赔清单" name="exportSettleList" class="bigbutton" onclick="window.target='_blank';window.location='plantingSettleExport.do?settleListCode=<%=settleListCode%>&&listType=<%=listType%>&&nodeType=<%=nodeType%>&&insureListCode=<%=insureListCode%>&&riskCode=<%=riskCode%>&&areaCode=<%=areaCode%>&&policyNo=<%=policyNo%>';"  style="display:none">
    </td>
   	<td ></td>
   </tr>
</table>

...
<%@page import="com.gyic.claim.dto.domain.Planting31SettleListTempDto" %>
<%@page import="com.gyic.claim.dto.domain.Planting31PolicyListDto" %>
...
<script language="javascript">
  ...
  function deleteItems(){
  	...
  	if("PolicyList"==dataSource){
  		...
  		if(nodeType==realNodeType){
			...
			if(checked){
		  		fm.action = "/claim/deletePlanting31SettleList.do"
		  		fm.submit();
	  		}else{
	  			alert("请至少选择一个农户理赔信息！");
	  			return false;
  ...
  function importSettle(){
	...
	if (!res)
		...
		document.fm.action="/claim/planting31SettleImport.do?";
 ...
 function save(){
 	...
 	if("loadXLS"!=loadXLS){
		...
		initScroll('scrlContainer', 'scrlContent');
 		document.fm.action="/claim/planting31SettleSave.do?";
 		document.fm.submit();
 	}
 }
...
<%	
	...
	if(null!=nodeType && !"".equals(nodeType)){
		String[] nodeList = com.gyic.claim.ui.control.facade.UIClaimGet31SettleListFacade.nodeList;
		String[] nodeListName = com.gyic.claim.ui.control.facade.UIClaimGet31SettleListFacade.nodeNameList;
...
%>
  <body onLoad="initPage()" >
     <form name="fm" action="/claim/planting31SettleList.do" method="post" enctype="multipart/form-data">
<table class="common">
...
<table id="table1" width="100%" class="common">
     ...
     if("SettleList".equals(listType) && !"loadXLS".equals(loadXLS)){//listTable
     	...
     	if(null!=collectionPlanting31SettleList && collectionPlanting31SettleList.size()>0){
     		...
     		int i=1;
     		while(it.hasNext()){
     			Planting31SettleListDto planting31SettleListDto = (Planting31SettleListDto)it.next();
     %>
     <tr class="listeven">
		<td>
	...
	<%		i++;
		...
		if(null != collectionPlanting31PolicyList && collectionPlanting31PolicyList.size()>0){
			...
			int i = 1;
			while(it.hasNext()){
				Planting31PolicyListDto planting31PolicyListDto = (Planting31PolicyListDto)it.next();
     ...
     <tr class="listeven">
		...
		<td>
		<%=planting31PolicyListDto.getZhiBuKa()==null?"&nbsp;":planting31PolicyListDto.getZhiBuKa()%>
		</td>
		<td id="insuredArea<%=i %>">
		<%=planting31PolicyListDto.getInsureArea()==0?"0":planting31PolicyListDto.getInsureArea()%>
		</td>
		<td id="sumInsured<%=i %>">
	...
	<%		i++;
		...
		//listTable
     	...
     	if(null!=collectionPlanting31SettleListTemp && collectionPlanting31SettleListTemp.size()>0){
     		...
     		int i=1;
     		while(it.hasNext()){
     			Planting31SettleListTempDto planting31SettleListTempDto = (Planting31SettleListTempDto)it.next();
     %>
     <tr class="listeven">
		<td>
...
%>
   ...
   <tr align="center" id="buttonTr">
    ...
    <td>
		<input type="button" value="导出理赔清单模板" name="exportTemplate" class="bigbutton" onclick="window.target='_blank';window.location='planting31SettleExport.do?settleListCode=<%=settleListCode%>&&listType=<%=listType%>&&nodeType=<%=nodeType%>&&insureListCode=<%=insureListCode%>&&riskCode=<%=riskCode%>&&areaCode=<%=areaCode%>&&template=template'; " style="display:none">
	</td>
	<td>
		<input type="button" value="导出理赔清单" name="exportSettleList" class="bigbutton" onclick="window.target='_blank';window.location='planting31SettleExport.do?settleListCode=<%=settleListCode%>&&listType=<%=listType%>&&nodeType=<%=nodeType%>&&insureListCode=<%=insureListCode%>&&riskCode=<%=riskCode%>&&areaCode=<%=areaCode%>';"  style="display:none">
    </td>
   	<td ></td>
   </tr>
</table>

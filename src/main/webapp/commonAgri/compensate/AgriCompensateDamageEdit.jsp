   ...
   <%=strTr %>
	...
	<td class="common" <%=strColspan %>>
		...
		%>
			...
			%>
					<%=strZ %><input type="checkbox" name="damageMessage" value="z999" checked disabled> 多种原因混合灾害
			<%
				}else{
			%>
					<%=strZ %><input type="checkbox" name="damageMessage" value="z999"  disabled>多种原因混合灾害
			<%		
				}
			if(PrpDcodeDtoList.size() > 15){
			%>

...
%>
          ...
          fm.prpLlossDtoDeductibleRate[<%=i %>].value       = '<%=10.0%>';
          <%}else if("3149".equals(riskcode) && Integer.parseInt(prpLlossDto1.getPolicyNo().substring(11,15))<=2016 && prpLlossDto1.getIndemnityDutyRate()>=90) {%>
          fm.prpLlossDtoDeductibleRate[<%=i %>].value       = '<%=0.0%>';
          <%}else if("3149".equals(riskcode) && Integer.parseInt(prpLlossDto1.getPolicyNo().substring(11,15))<=2016 && prpLlossDto1.getIndemnityDutyRate()<90) {%>
          fm.prpLlossDtoDeductibleRate[<%=i %>].value       = '<%=10.0%>';
          <%}else if("3149".equals(riskcode) && Integer.parseInt(prpLlossDto1.getPolicyNo().substring(11,15))>2016) {%>
          fm.prpLlossDtoDeductibleRate[<%=i %>].value       = '<%=0.0%>';
          <%} else if("3140".equals(riskcode)){%>
          fm.prpLlossDtoDeductibleRate[<%=i %>].value       = '<%=prpLlossDto1.getDeductibleRate()%>';
          <%} else if(prpLlossDto1.getIndemnityDutyRate()>=80 && !"3224".equals(riskcode) && !"3130".equals(riskcode) && !"3149".equals(riskcode)){%>
          fm.prpLlossDtoDeductibleRate[<%=i %>].value       = '<%=0.0%>';
          <%} else{%>
          fm.prpLlossDtoDeductibleRate[<%=i %>].value       = '<%=prpLlossDto1.getDeductibleRate()%>';
          <%}%>

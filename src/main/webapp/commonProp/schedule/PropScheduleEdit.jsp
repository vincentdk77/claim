...
<meta http-equiv="Pragma" CONTENT="no-cache">
  ...
  <script src="/claim/commonProp/schedule/js/PropScheduleEdit.js"></script>
...
//--> 
    ...
    <input type="hidden" name="org.apache.struts.taglib.html.TOKEN" value="<%= session.getAttribute("org.apache.struts.action.TOKEN") %>">
     <%-- 1.调度主信息头信息 --%>
      <%@include file="/commonProp/schedule/PropScheduleMainEdit.jsp" %>
      <%  
          //判断是否无查勘
          String noCheck="1";
      %>
    ...
    %>  <%-- 2.查勘调度标的内容,如果设置了no查勘，则给出标记 --%>
      ...
      <%--查勘调度处理界面--%>
      <%@include file="/commonProp/schedule/PropScheduleCheckItemEdit.jsp" %>
      <%noCheck="0";%>
      
      <%-- 财产险不需要定损调度
      3.定损调度标的内容
      <logic:equal name="prpLscheduleMainWFDto"  property="scheduleType" value="ALLS">
        <%@include file="/commonProp/schedule/PropScheduleItemEdit.jsp" %>
      </logic:equal>
       --%>
      <%-- 4.报案信息补充说明 --%>
...
%>  
     <%-- 5.调度保存按钮 --%>
     <%@include file="/commonProp/schedule/PropScheduleSave.jsp" %>  

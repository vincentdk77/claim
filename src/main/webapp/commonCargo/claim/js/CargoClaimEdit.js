...
function sumLossAmountInformCargo() {
         var returnString ="";
...
function saveForm(field,saveType)
{
 // 校验估损金额
  ...
  if(isEmptyField(fm.prpLextSalvor)){ 
    errorMessage = errorMessage + "货主名称不允许为空，请先完成查勘任务\n";
  }    
  ...
  //add by zhulei begin 20050716 出险时间不能早于起运时间的比较校验
  if(fm.prpLclaimDamageStartDate.value<fm.prpLclaimStartDate.value)
  {
  	alert("错误！出险时间不能早于起运时间");
  	return false;
  }
  //add by zhulei end 20050716 出险时间不能早于起运时间的比较校验
  ...
  if(confirm(sumLossAmountInformCargo() + "\n确认继续提交吗？")) {
      fm.submit();
      return true;   
  } else {
...
//险别和币别相同的，必须合并
  	for( var n=j+1;n<fm.prpLclaimLossCurrency.length;n++)
 		{
 			if(fm.riskcode.value!="1003"){
				if ((fm.prpLclaimLossCurrency[n].value==fm.prpLclaimLossCurrency[j].value)&&(fm.prpLclaimLossItemCode[n].value==fm.prpLclaimLossItemCode[j].value)&&(fm.prpLclaimLossLossFeeType[n].value==fm.prpLclaimLossLossFeeType[j].value)&& fm.prpLclaimEditType.value != "EDIT")
				{
					errorMessage("第"+ n+"条估损金额中项目及币别及费用类别和第"+ j+"条估损金额中项目及币别及费用类别一致，请合并这2条记录");			
				
					fm.prpLclaimLossItemCode[j].focus();
				
					return false;
				}
			}else{
				if ((fm.prpLclaimLossCurrency[n].value==fm.prpLclaimLossCurrency[j].value)&&(fm.prpLclaimLossKindCode[n].value==fm.prpLclaimLossKindCode[j].value)&&(fm.prpLclaimLossLossFeeType[n].value==fm.prpLclaimLossLossFeeType[j].value)&& fm.prpLclaimEditType.value != "EDIT")
				{
					errorMessage("第"+ n+"条估损金额中项目及币别及费用类别和第"+ j+"条估损金额中项目及币别及费用类别一致，请合并这2条记录");			

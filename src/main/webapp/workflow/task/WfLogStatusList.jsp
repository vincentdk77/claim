...
<%@page import="com.sinosoft.claim.dto.domain.PrplcompensatehouseDto"%>
<%   
    ...
    <tr>  <td colspan=4 class="formtitle">查询条件</td></tr>
      <tr>
        <td class='title'>
           ...
           <% if (searchLabel.equals("specialNo")) subTitleColName="预赔案号:"; %>
           <%=subTitleColName%>
           <% 
             //--------------------------------------
      ...
      <tr>
        ...
        <%
		            String statStartDate = "";
		       		...
		       		statStartDate = dateTime.addDay(-7).toString();
		       		statEndDate = dateTime.toString();
		   
		       
		           %>
         <td class='title' colspan=2><%=subTitleTime%>:
          <input type=text style="width:85" width="30%" name="statStartDate" value="<%=statStartDate %>" class="Wdate" onfocus="WdatePicker()">
          到：
          <input type=text style="width:85" name="statEndDate" value="<%=statEndDate %>" class="Wdate" onfocus="WdatePicker()"> 
       </td> 
      </tr>
      <!--------------------增加被保险人，报案号查询-start---------------------------------------------->
      <tr>
								...
								if (!nowNodeStatus.equals("0")){
									if(nodeStatus.equals("-1")&&funcName!=null&&funcName.equals("specialApply")){
										...
										dealHref="/claim/claimBeforeCancel.do?ClaimNo="+swfLogDto.getKeyOut()
											+ "&RegistNo="+swfLogDto.getKeyIn()+flowStr;
									}
									dispHref=swfLogDto.getBusinessNo();
								}else{
									if (nodeStatus.equals("-1")&&funcName!=null&&funcName.equals("specialApply")){
										...
										dealHref="/claim/claimBeforeCancel.do?RegistNo="+swfLogDto.getBusinessNo()
											+ "&type=" + request.getAttribute("com.sinosoft.type")+flowStr;
                              		}
                             	dispHref=swfLogDto.getBusinessNo();
								}
							}else{
...
<%                 if ((nodeStatus.equals("0")&&nowNodeStatus.equals("0"))){
 ...
 %>
        ...
        <logic:notEqual name="swfLogList1" property="riskCode" value="0310">
        <logic:notEqual name="swfLogList1" property="riskCode" value="0312">
        <td align="center" width="10%"><bean:write name="swfLogList1" property="insuredName"/></td> 
        </logic:notEqual >
        </logic:notEqual >
        <logic:equal name="swfLogList1" property="riskCode" value="0310">
        <%	
        	registNo0310 = swfLogDto.getRegistNo();
            //System.err.println(registNo0310);
        	nodeType0310 = swfLogDto.getNodeType();
        	//System.err.println(nodeType0310);
			...
			String condition = " registno='" + registNo0310 + "' and nodetype='"+nodeType0310+"'";
			...
			if(prplCompensateHouseDtoList != null && prplCompensateHouseDtoList.size() >0)
			{
					...
					PrplcompensatehouseDto PrplcompensatehouseDto = (PrplcompensatehouseDto)prplCompensateHouseDtoList.get(0);
					insuredName0310 = PrplcompensatehouseDto.getName();
					if(!insuredName0310.equals("")) 
					{
						insuredName0310 = "("+insuredName0310+")";
					}
				
			}else
			{
				condition = " registno='" + registNo0310 + "'";
				prplCompensateHouseDtoList = (ArrayList)bLPrplcompensatehouseFacade.findByConditions(condition);
				if(prplCompensateHouseDtoList != null && prplCompensateHouseDtoList.size() >0)
				{

						PrplcompensatehouseDto PrplcompensatehouseDto = (PrplcompensatehouseDto)prplCompensateHouseDtoList.get(prplCompensateHouseDtoList.size()-1);
						insuredName0310 = PrplcompensatehouseDto.getName();
						if(!insuredName0310.equals("")) 
						{
							insuredName0310 = "("+insuredName0310+")";
        ...
        %>
        <td align="center" width="10%"><bean:write name="swfLogList1" property="insuredName"/><%=insuredName0310%></td> 
        </logic:equal >
        
        <logic:equal name="swfLogList1" property="riskCode" value="0312">
        <%	
        	registNo0310 = swfLogDto.getRegistNo();
            //System.err.println(registNo0310);
        	nodeType0310 = swfLogDto.getNodeType();

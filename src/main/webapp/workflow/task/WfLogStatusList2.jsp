...
String RiskClaimType1 = (String)request.getAttribute("RiskClaimType1");
String RiskClaimType2 = (String)request.getAttribute("RiskClaimType2");
...
<%
   ...
   String[] riskCode1 = new String[riskCodeCollection.size()];
   ...
   for(int i=0; i<riskCodeCollection.size(); i++)
   {
       ...
       riskCode1[i] = prpdRiskDto.getRiskCode();
...
<head>
  ...
  function submitForm()
  {
    if(fm.RiskClaimType!=null){
       if(trim(fm.RiskClaimType.value).length==0){
         alert("险种大类不能为空,请选择！！");
         return false;
       }else{
          if(fm.prpLregistRiskCode!=null&&trim(fm.prpLregistRiskCode.value).length==0){
             alert("险种不能为空,请选择！！");
             return false;
          }
       }
...
<form name="fm" action="/claim/wfLogQuery.do"  method="post" onsubmit="return validateForm(this);">
    ...
    <tr>  <td colspan=4 class="formtitle">查询条件</td></tr>
      ...
      <tr>
       <%
        ...
        <td class='title' colspan=2><%=subTitleTime%>:
          <input type=text style="width:85" width="30%" name="statStartDate" value=<%=statEndDate %> class="Wdate"  onfocus="WdatePicker()"> <!-- <img style='cursor: hand' src="/claim/images/bgcalendar.gif"  align="absmiddle" onclick="TogglePopupCalendarWindow('document.fm.statStartDate', '2007', '2017')"> -->
          到：
      ...
      <!--------------------增加被保险人，报案号查询-start---------------------------------------------->
      <tr>
      <%  System.out.println("searchLabel---------------------1----------"+searchLabel);
       if ( !(searchLabel.equals("compensateNo") && nodeType.equals("compp")) && 
              ...
              !(searchLabel.equals("businessNo") && nodeType.equals("compe") )
            ) { %> 
        <% System.out.println("---------------------2---------");
          signselect="";
          if (insuredNameSign1.equals("=")) signselect="selected";
         %>  
        <td class='title'>被保险人q11：</td>
        <td class='input' >
          <select class=tag name="insuredNameSign"  value="<%=insuredNameSign1%>">
            <option value="*">*</option>
            <option value="=" <%=signselect%>>=</option>
...
%>        
      ...
      <% if (searchLabel.equals("compensateNo") && nodeType.equals("compp") ) { %>
        ...
        <td class='title' rowspan="3">险种：</td>
       ...
       <% } else if (searchLabel.equals("claimNo") && nodeType.equals("compe") ) {   %>
         ...
         <td class='input'>
          <select class=tag name="CompeRegistNoSign"  value="<%=searchContentSign5%>">
            <option value="=" <%=signselect%>>=</option>
          </select> <input type=text name="CompeRegistNo" class="query"  style="width:70%"   value="<%=searchContent5%>">
        </td>
         <td class='title' rowspan="3">险种：</td>
        <td class="input" rowspan="3">
            <select class=tag name="RiskCodeSign" style="display:none">
      ...
      <td>
          ...
          <select name="RiskClaimType" class="query" style="width: 70%" onchange="buildRiskCodeSelect(fm.RiskClaimType, fm.prpLregistRiskCode);">
        ...
        <% if (searchLabel.equals("businessNo") && nodeType.equals("compe") ) { %>
          <td class='title' rowspan="2">险种：</td>
      ...
      <tr>
        <td colspan="3" class='input'><font color="red">为了提高您的查询速度，请尽量录入精确的查询条件。同时系统不允许险种条件为空的查询</font></td>
      </tr>
      <!--------------------增加被保险人查询----end------------------------------------------->
      <tr>
...
%>
 ...
 <%
  ...
  <script language="javascript">
    ...
    <%for(int i=0; i<riskCodeCollection.size(); i++){%>
        riskCodes[<%=i%>] = new Array("<%=riskCategory[i]%>", "<%=riskCode1[i]%>", "<%=riskName[i]%>");
    <%}%>
    ...
    <%for(int i=0; i<riskCodeCollection.size(); i++){%>
        riskCodes[<%=i%>] = new Array("<%=riskCategory[i]%>", "<%=riskCode1[i]%>", "<%=riskName[i]%>");
    <%}%>
    var riskCategory = fm.RiskClaimType;
    for(var i=0; i<riskCategory.length; i++){
        if(riskCategory.item(i).value=="<%=RiskClaimType1%>"){
            riskCategory.item(i).selected = true;
        }
    }
    fm.RiskClaimType.onchange();
    var riskCodeField = fm.prpLregistRiskCode;
    for(var i=0; i<riskCodeField.length; i++){
        if(riskCodeField.item(i).value=="<%=RiskClaimType2%>"){

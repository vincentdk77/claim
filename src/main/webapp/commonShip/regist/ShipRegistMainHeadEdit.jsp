...
<!--modify by liuyanmei add 20051114 end-->
    ...
    <tr>
     ...
     <%if(!editType.equals("ADD")){%>
       <td><input type="button" name="messageSave" class="bigbutton" value="撰写留言" onclick="openWinSave('<bean:write name='prpLregistDto' property='registNo'/>','<bean:write name='prpLregistDto' property='policyNo'/>','<bean:write name='prpLregistDto' property='riskCode'/>','regis','')"></td>
     <%}else{%>
       <td><input type="button" name="messageSave" disabled class="bigbutton" value="撰写留言" onclick="openWinSave('<bean:write name='prpLregistDto' property='registNo'/>','<bean:write name='prpLregistDto' property='policyNo'/>','<bean:write name='prpLregistDto' property='riskCode'/>','regis','')"></td>
     <%}%> 
...
<%--  
     ...
     --%>
      <tr>
        ...
        <td class="input">
          <input  name="prpLregistReportDate" class="Wdate"  maxlength="10" style="width:80px"  onfous="WdatePicker()"    value="<bean:write name='prpLregistDto' property='reportDate' filter='true' />">日
      ...
      <%@ include file="/commonShip/regist/ShipRegistLinkmanEdit.jsp" %>
      <!--modify by wangwei add end 20050531-->

      <tr>
        <td class="title">出险时间:</td>
        <td class="input">
          <input type=text name="prpLregistDamageStartDate" onfocus="WdatePicker()" title="出险时间" class="Wdate" maxlength="10" style="width:80px" value="<bean:write name='prpLregistDto' property='damageStartDate' filter='true' />" >日
      ...
      <!--modify by liuyanmei modify 20051025 start -->
      <!-- reason:隐藏：报损费用 200508-011 -->
      <tr>
        <td class="title"><%--<bean:message key="db.prpLregist.estimateLoss" />--%>报损金额:</td>
        <td class="input" colspan=3>
        <input type=text name="prpLregistEstimateLoss" title="报损金额" Class="input" style="width:80px" value="<bean:write name='prpLregistDto' property='estimateLoss' filter='true' format='0.00'/>">
        <!--</td>-->

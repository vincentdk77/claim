		...
		if(this.checked){
			...
			if("P60"==payrefreason||"P50"==payrefreason){//赔款
				...
				if(receiverType=="t01"||receiverType=="t02"||receiverType=="t03"||receiverType=="t04"||receiverType=="t06"
						...
						||receiverType=="t19"||receiverType=="t20"){
					alert("领款人为“"+receiverTypeName+"”时,不允许支付“"+payrefreasonName+"”");
					checkFlag=true;
				}
...
<body class="interface"  onload="getPayTotalAmount();proBank();<c:if test="${editType eq 'SHOW'}">readonlyAllInput();readonlyAllCheckbox();</c:if>" >
	<form name="fm" action="/claim/payGetherSave.do" method="post" onsubmit="return validateForm(this);">
		...
		<div class="outcontainer" id="payGetherMain">
			<%-- 基本信息 --%>
	       	<%@include file="/DAA/payment/DAAPayBaseInfo.jsp" %>
	       	<%-- 账户信息 --%>
	       	<%@include file="/DAA/payment/DAAPayAccountInfo.jsp" %>
	       	<!-- 领款人信息 -->	
		    <%@include file="/DAA/payment/DAAPayReceiverInfo.jsp"%>
		    <%-- 赔付信息 --%>
	       	<%@include file="/DAA/payment/DAAPayMainInfo.jsp" %>
		    <%-- 处理意见 --%>

...
return  checkMinlength(document.getElementById("bankAccount"));
checkBankAccount();return  checkMinlength(document.getElementById("bankAccount2"));
}
function data10(c){
document.getElementById("citySelect").value=trim(c);
}
//by goudexuan 2017/12/7 给 银行大类,银行账号,账号确认,账号属性,账号类型 赋值  end
...
<div id="content_basic" style="width: 98%" name="accountInfoDiv">
	...
	<table class="common" id="accountInfo" cellpadding="0" cellspacing="0" border="1" bordercolor="#3A81C8">
		<tr>
			...
			<input id="btn_hukou" name="cityCode" style="width:" type="text"  onclick="hukouSelect()" value="${prpLpayGatherDto.cityCode }"/>
			</c:if>
			<c:if test="${editType eq 'SHOW' }">
			<input id="btn_hukou" name="cityCode" style="width:" type="text"   readOnly value="${prpLpayGatherDto.cityCode }"/>
			</c:if>
			<!-- 
		...
		<tr>
            ...
            <c:if test="${editType ne 'SHOW' }">
			<div class="inputbox">
				<input name="bankType" type="text" value="${prpLpayGatherDto.bankType}" id="citySelect" onclick="suggestF.display(this,'',event);" onkeydown="item_suggest.item_display(this,'',event);" onblur="item_suggest.item_onblur(this);" onpropertychange="proBank();" class="stext" readonly="readonly"/>
			</div>
            </c:if>	
            <c:if test="${editType eq 'SHOW' }">
			...
			<!-- <div class="inputbox1"> -->
			<div>
				<input name="bank1" style="width: 99%" value="${prpLpayGatherDto.bank}"   id="bank1"  onblur="proBank();" onkeyup="item_suggest1.item_display1(this,'',event);" class="stext" />
				<input name="bank" type="hidden"  value="${prpLpayGatherDto.bank}"   id="bank"/>
			</div>
			</td>
		...
		<tr>
			...
			<td class="input" width="20%">
				<div id="main">
				<input type="text" style="width: 99%" maxlength="30"  class="input" id="bankAccount" name="bankAccount" value="${prpLpayGatherDto.bankAccount}"  onblur="clearNoNumZ(this);return checkMinlength(this);"/>
			...
			<td class="input" width="20%">
			<div id="main">
				<input type="text" style="width: 99%" maxlength="30" class="input" id="bankAccount2" name="bankAccount2" value="${prpLpayGatherDto.bankAccount}" onblur="checkBankAccount();return checkMaxlength(this);"/>
				<div class="text-magnifier fn-hide" id="textMag"  >
				<div id="mag-text" class="mag-text"></div>
				<div class="mag-explain">银行卡号码最多是30位数字</div>
			</td>
			...
			<td class="input" width="14%" colspan="2">
				<div id="accountFlag0" <c:if test="${prpLpayGatherDto.accountType eq '2' }">style="display:none"</c:if>>
					<input onchange="proBank();" type="radio" style="border: 0px;" id="accountFlag" name="accountFlag" <c:if test="${prpLpayGatherDto.accountFlag eq '00' }">checked='checked'</c:if> value="00">银行卡
					<input onchange="proBank();" type="radio" style="border: 0px;" id="accountFlag" name="accountFlag" <c:if test="${prpLpayGatherDto.accountFlag eq '01' }">checked='checked'</c:if> value="01">存折
					<!-- <input onchange="proBank();" type="radio" style="border: 0px;" id="accountFlag" name="accountFlag" <c:if test="${prpLpayGatherDto.accountFlag eq '02' }">checked='checked'</c:if> value="02">信用卡 -->
				</div>
				<div id="accountFlag1" <c:if test="${prpLpayGatherDto.accountType eq '1' }">style="display:none"</c:if>>
					<input onchange="proBank();" type="radio" style="border: 0px;" id="accountFlag" name="accountFlag" <c:if test="${prpLpayGatherDto.accountFlag eq '03' }">checked='checked'</c:if> value="03"/>对公账户
				</div>
			</td>
			<td class="input" width="4%" align="center">

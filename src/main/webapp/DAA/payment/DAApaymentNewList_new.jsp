...
<script  type="text/javascript">
	function mergeBillPay(){
	    ...
	    var compensateNo = document.getElementsByName('compensateNo');
		var selectcompensateNoArr = new Array();
	    var selectRegistNo = "";
		for(var i=0;i<registNo.length;i++){
			if(registNo[i].checked == true){
				selectRegistNo = registNo[i].value;
				selectcompensateNoArr.push(compensateNo[i].value);
			}
		}
		selectcompensateNoArr.sort();
		for(var i=1;i<selectcompensateNoArr.length;i++){
		   if(selectcompensateNoArr[i-1]!=selectcompensateNoArr[i]){
		   	  alert("只能选择一个计算书！");
			  return false;
		   }
		}
		...
		var flag1 = false;
		$.ajax({
			url:'/claim/CheckSumthispaid.do'
			,type:'POST'
			,dataType:"text"
			,async:false
			,contentType: "application/x-www-form-urlencoded; charset=utf-8" 
			,data:{"compensateNo":selectcompensateNoArr[0]}
			,success:function(data){
				var retMsg = data.split(",");
				var flag = retMsg[0];
				var sumthispaid = retMsg[1];
				if(flag=="true"){
				   if(sumthispaid<=0){
				      alert("该计算书本次赔付金额小于等于0,不允许清单支付！");
				      flag1 =true;
				   }
				}
			}
		...
		if(!isBill && type == '5'){
			if(confirm("勾选的案件无清单，是否继续进行“清单支付”？")){
				...
				fm.action="/claim/compensateListNew.do?editType=ADD&Datatype=mergeMany&nodeType=apay&selectBillFlag=1&noBillFlag=1";
				fm.submit();
				return false ; 
			}else{
				return false;
		...
		x_load.show("正在执行中...");
		if(isBill&&isBillNull){
			fm.action="/claim/compensateListNew.do?editType=ADD&Datatype=mergeMany&nodeType=apay";
			fm.submit();
		}else{
  ...
  <body>
  <form action="/claim/prplpaynewadd.do" name="fm" method="post">

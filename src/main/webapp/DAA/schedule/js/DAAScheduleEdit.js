...
function saveForm(field)
{
  ...
  */
  ...
  //控制承保时没有保人伤、标的、财产、三者时，不能进行相应的定损
  if(fm.all("checkYesNo") != null){
    var havePersonKind = "";
    var haveThirdCarKind = "";
    var havePropKind = "";
    var haveCarKind = "";
    if(fm.havePersonKind != null){
        havePersonKind = fm.havePersonKind.value;
    }
    if(fm.haveThirdCarKind != null){
        haveThirdCarKind = fm.haveThirdCarKind.value;
    }
    if(fm.havePropKind != null){
        havePropKind = fm.havePropKind.value;
    }
    if(fm.haveCarKind != null){
        haveCarKind = fm.haveCarKind.value;
    }
    var checkBoxCount = getElementCount("checkYesNo");
    var scheduleCount = getElementCount("prpLscheduleItemItemNo");
    if(checkBoxCount == 1){
        for(var i = 0; i < checkBoxCount;i++){     
            if(fm.checkYesNo != null && fm.checkYesNo.checked == true){
                if(fm.prpLscheduleItemItemNo != null){
                    if(fm.prpLscheduleItemItemNo.value == "-1" && havePropKind == "false"){
                        if(confirm("该保单未承保财产相关的险别，不应对“财产损失”进行定损调度处理\n\n确定要继续吗") == false){
                            return false;
                        }
                    }else if(fm.prpLscheduleItemItemNo.value == "0" && havePersonKind == "false"){
                        if(confirm("该保单未承保人伤相关的险别，不应对“人伤”进行定损调度处理\n\n确定要继续吗") == false){
                            return false;
                        }
                    }else if(fm.prpLscheduleItemItemNo.value == "1" && haveCarKind == "false"){
                        if(confirm("该保单未承保标的车相关的险别，不应对“标的车”进行定损调度处理\n\n确定要继续吗") == false){
                            return false;
                        }
                    }else if(fm.prpLscheduleItemItemNo.value == "2" && haveThirdCarKind == "false"){
                        if(confirm("该保单未承保三者车相关的险别，不应对“三者车”进行定损调度处理\n\n确定要继续吗") == false){
                            return false;
                        }
                    }
                }                
	    ...
	    for(var i = 0; i < checkBoxCount;i++){	   
	        if(fm.all("checkYesNo")[i] != null && fm.all("checkYesNo")[i].checked == true){
	            if(fm.all("prpLscheduleItemItemNo")[i] != null){
	                 if(fm.all("prpLscheduleItemItemNo")[i].value == "-1" && havePropKind == "false"){
	                     if(confirm("该保单未承保财产相关的险别，不应对“财产损失”进行定损调度处理\n\n确定要继续吗") == false){
                            return false;
                        }
	                 }else if(fm.all("prpLscheduleItemItemNo")[i].value == "0" && havePersonKind == "false"){
                        if(confirm("该保单未承保人伤相关的险别，不应对“人伤”进行定损调度处理\n\n确定要继续吗") == false){
                            return false;
                        }
                    }else if(fm.all("prpLscheduleItemItemNo")[i].value == "1" && haveCarKind == "false"){
                        if(confirm("该保单未承保标的车相关的险别，不应对“标的车”进行定损调度处理\n\n确定要继续吗") == false){
                            return false;
                        }
                    }else if(fm.all("prpLscheduleItemItemNo")[i].value == "2" && haveThirdCarKind == "false"){
                        if(confirm("该保单未承保三者车相关的险别，不应对“三者车”进行定损调度处理\n\n确定要继续吗") == false){
                            return false;
                        }
                    }
...
//2.检查选择了checkbox的查勘信息和出险地址都需要填内容。	
  ...
  if ((maxRow==1)||!(fm.checkYesNo.length>0))
  {
    ...
    if ((fm.checkYesNo.checked==true) &&(fm.checkScheduleCheckYesNo.checked==false))
  	{
     	errorMessage("标的车辆，选择定损调度时，也需要选择查勘调度！")
   	  return false;
    }
...
function CheckOnlySchel()
{
 //设置selectSend的内容
        ...
        //判断定损地址
        if (!fm.prpLscheduleItemCheckSite==undefined&&((fm.prpLscheduleItemCheckSite.value).toString().length)<1)
        {

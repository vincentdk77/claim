...
/****在新增标准配件前作相应的控制****/
function checkBeforeGetFittingsInfo(){
	if(isEmptyField(fm.prpLcarLossLossItemName)){
		errorMessage("号牌号码不能为空！");
		return false;
	}
	//新增标准配件
	getFittingsInfo('Compent');
}
...
function deleteRowTableRepairFee(field,pageCode,pageDataRowsCount,controlRowsCount)
{
  ...
  // 国元核损时将核损信息归到一行，多多行输入域处理时不需要下面的代码 Delete BY YEHUIQUAN 2009-07-08
  /*
...
function deleteRowTableComponent(field,pageCode,pageDataRowsCount,controlRowsCount)
{
  ...
  var order = recentDeletedRowNo - 2;  //顺序改为以0开始
  if(document.getElementsByName(pageCode)[pageSerialNo] == "undefined" || document.getElementsByName(pageCode)[pageSerialNo] == null){
    pageSerialNo = 0; 
  }
  var oTBODY   = document.getElementsByName(pageCode)[pageSerialNo].tBodies.item(0);
  var computLine=0;

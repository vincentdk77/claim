...
<link rel="stylesheet" type="text/css" href="/claim/css/StandardCar.css">
	...
	<script type="text/javascript">
		...
		function saveForm(flag){
			...
			if(flag=="5"){
				fm.action="/claim/common/payment/BillPaymentExcel_new.jsp";
				fm.submit();
			}
			if(flag=="7"){
				fm.action="/claim/common/payment/BillPaymentNoBillExcel_new.jsp";
				fm.submit();
			}
			if(flag=="6"){
			...
			if(flag=='4'||flag=='2'){
			$.ajax({
				url:'/claim/prplBilElementCheckNew.do'
				,type:'POST'
				,dataType:"text"
				,contentType: "application/x-www-form-urlencoded; charset=utf-8" 
				,data:$(document.fm).serialize()
...
function impExcel(){
	...
	$("input[name='prpLpayExtContext2']:last").val(direction);
	var submitType = "4";
	if(submitType!=null&&submitType!=""){
		if(submitType=="2"){
			$("input[name='prpLpayExtNotionCode2']:last").val("1");
			$("input[name='prpLpayExtNotionName2']:last").val("暂存任务");
		}else if(submitType=="4"){
			var vflag = fm.vflag.value;
			if(vflag!=null&&vflag!=""){
				if(vflag=="3"){
					$("input[name='prpLpayExtNotionCode2']:last").val("3");
					$("input[name='prpLpayExtNotionName2']:last").val("已补充信息");
				}else{
					$("input[name='prpLpayExtNotionCode2']:last").val("2");
					$("input[name='prpLpayExtNotionName2']:last").val("提交审核");
				}
			}else{
				$("input[name='prpLpayExtNotionCode2']:last").val("2");
				$("input[name='prpLpayExtNotionName2']:last").val("提交审核");
	...
	if(confirm("数据导入后系统将自动保存数据，请仔细确认表格填写是否无误，是否继续导入？")){
		x_load.show('页面加载中...');
		fm1.action="/claim/prplBIllExcelNew.do";
		fm1.submit();
	}
...
prpallUtility.getPhisicaMemory("数据初始化加载-----begin"); %>
  ...
  <form action="/claim/prplBIllExcelSaveNew.do" name="fm"  method="post" >
		...
		<c:if test="${not empty prpLclaimBillManagerList}">
   		<table  class="common" cellpadding="0" cellspacing="0" bordercolor="#3A81C8" id="baseInfo" border="1" style="width: 98%">
   			<tbody>
   					<tr>
		   				...
		   				<td class="title" style="width: 15%">被保险人名称</td>
		   				<td class="title" style="width: 15%">理算书号</td>
	    		...
	    		<tr>
			    	...
			    	<td class="input"   style="width: 10%">
			    		<input type="text"  class="readonly" readonly name="compensateNo" value="${billManager.compensateNo }">
			    	</td>
			    	<td class="input"   style="width: 10%">
			    		<input type="text"  class="readonly" readonly name="sumPaid" value="<fmt:formatNumber type='number' value='${billManager.sumPaid }' pattern='#.00' maxFractionDigits='2'/>">
			    	</td>
			    	<td class="input"   style="width: 10%">
			    		<input type="text"  class="readonly" readonly name="payAmount" value="<fmt:formatNumber type='number' value='${billManager.havPaid}' pattern='#.00' maxFractionDigits='2'/>">
			    	</td>
			    	<c:if test="${isAccBack}">
	   					<td class="input" style="width: 10%">
	     ...
	     <table  class="common" cellpadding="0" cellspacing="0" bordercolor="#3A81C8" id="baseInfo" border="1" style="width: 98%">
	    		...
	    		<tr>
					...
					<td class="input"  style="width: 15%">${status.count }--
			    	...
			    	<td class="input"   style="width: 10%">
			    		<input type="text"  class="readonly" readonly name="sumPaid" value="<fmt:formatNumber type='number' value='${billPay.payAmount }' pattern='#.00' maxFractionDigits='2'/>">
			    	</td>
			    	<td class="input"   style="width: 10%">
			    		<input type="text"  class="readonly" readonly name="payAmount" value="<fmt:formatNumber type='number' value='${billPay.payAmount }' pattern='#.00' maxFractionDigits='2'/>">
			    	</td>
			    	<c:if test="${isAccBack}">
	   					<td class="input" style="width: 10%">
	...
	<form action="/claim/prplBIllExcelSaveNew.do" name="fm1"  method="post" ENCTYPE="multipart/form-data">
		<input type="hidden" name="payType" value="P3"/>
		<input type="hidden" name="paymentType" value="P60"/>
  		<input type="hidden" name="submitType" value="">
		<input type="hidden" name="Datatype" value="mergeMany">

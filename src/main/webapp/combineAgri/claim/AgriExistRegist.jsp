...
<%
   String policyNo = (String)request.getAttribute("policyNo_"+indexMainHead);
   String curRegistNo = (String)request.getAttribute("curRegistNo_"+indexMainHead);
...
<input type=text name="PerilCount_<%=indexMainHead%>" class="readonly" readonly="true" style="width:50%;text-align='center';color:'#9B009B'" value="<bean:write name='prpLregistDto1' property='perilCount' filter='true' />">
<input type="hidden" name="checkFlag_<%=indexMainHead%>" value="<%=checkFlag %>">
<input title="点击此处可获得已出险相关信息"  type=button ACCESSKEY="." value='...' class="smallbutton" name='button_Peril_Open_Context_<%=indexMainHead%>' onclick="buttonOnClick('perilInfoShow',fm.policyno_<%=indexMainHead%>.value,fm.registno_<%=indexMainHead%>.value,'<%=indexMainHead%>');">
  ...
  <table class="prompt" style="width:400">
     ...
     <%-- <logic:notEmpty  name="prpLregistDto1"  property="registList"> 
        ...
        <%
            //modify by wangwei add start 2005-06-04
            //原因：从立案信息表中带出部分信息
            java.util.ArrayList registClaimDtoList = (java.util.ArrayList) request.getAttribute("registClaimDtoList_"+indexMainHead);
      ...
      <tr>  
        <td colspan=14 class="prompttitle" >
          <input type=button name = 'button_Peril_Close_Context_<%=indexMainHead%>' value='(O)关闭' class="button"  ACCESSKEY="O" onclick="hideSubPage(this,'span_Peril_Context_<%=indexMainHead%>')">  
        </td> 
      </tr>
  </table>        
</span>
...
<%
   int endorestimes = 0;
   ArrayList prpPheadDtoList = (ArrayList) request.getAttribute("prpPheadDtoList_"+indexMainHead);
   ...
   if(endorestimes > 0){
%>
<input title="点击此处可获得历次批单相关信息"  type=button ACCESSKEY="." class="button" value='历次批单' name='button_Peril_Open_Context2_<%=indexMainHead%>' onclick="buttonOnClick2('span_Peril_Context2_<%=indexMainHead%>');">
<%
   }
%>
    
<span id="span_Peril_Context2_<%=indexMainHead%>" style='width:800;display:none;position:absolute;background-color:FFFFFF;'> 
  <table  class=common cellpadding="5" cellspacing="1">
      <tr>
       ...
       <%
            //原因：从批改信息表中带出部分信息
            prpPheadDtoList = (ArrayList) request.getAttribute("prpPheadDtoList_"+indexMainHead);
      ...
      <tr>  
        <td colspan=14 class="common" >
          <input type=button name = 'button_Peril_Close_Context2_<%=indexMainHead%>' value='(O)关闭' class="button" ACCESSKEY="O" onclick="hideSubPage2(this,'span_Peril_Context2_<%=indexMainHead%>')">  
        </td> 
      </tr>
  </table>        
</span>    

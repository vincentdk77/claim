...
<head>
  ...
  {
    progressStart();
    fm.action="/claim/wfLogUniteQuery.do?Flag=N";
...
<%
     ...
     int colNumber=7;         //基本的列数 基本显示是7列
     String strClaimRiskType = request.getParameter("RiskClaimType");
    // String strClaimRiskType = "H";
     String checkStr="";
%>
<form name="fm" action="/claim/wfLogUniteQuery.do?Flag=Y"  method="post">
<input type="hidden" name="CombineLimit" value="<%= AppConfig.get("sysconst.COMBINE_LIMIT") %>">
      ...
      <tr>      
        ...
        <td class='input' >
          <select class=tag name="PolicyNoSign">
            <option value="*">*</option>
            <option value="=">=</option>
          </select> <input type=text name="PolicyNo" class="query"  style=style="width:70%">
        </td>
 ...
 <app:processBar/>
     ...
     <tr><td colspan=12 class="formtitle"> 未处理并案信息</td>
     </tr>
     <tr>
     ...
     if(index %2== 0)
      ...
      %>
         <%
           ...
           SwfLogDto swflog = (SwfLogDto) ((ArrayList)swfLogDto1.getSwfLogList()).get(index);
           String checked="";
           if(checkStr.contains(swflog.getRegistNo())){
        	   checked="checked";
           }
         %>
      ...
      %>
         ...
         <td align="center">
            <a href="/claim/registFinishQueryList.do?prpLregistRegistNo=<bean:write name='swfLogList1' property='registNo'/>&editType=SHOW&riskcode=3220"> <bean:write name="swfLogList1" property="registNo"/></a></td>
        </td>
        <td align="center">
     ...
     <tr>
       <td class='button' colspan="8">
          <input type=button name="comBineButton" class='button' value="案件合并" onclick="claimComBineCheck()">
        </td>
        </tr>   

...
function saveFormNew(field,saveType)
 {  disabledAllButton('buttonArea');
	...
	if ( bool != true) {enableAllButton('buttonArea');return false;}
	bool = saveForm(field,saveType);
	if ( bool != true) {enableAllButton('buttonArea');return false;}
	return true;
}
</script>
...
<table id="buttonArea" cellpadding="5" cellspacing="1" class=common style="display:" >
  ...
  <tr>  
        ...
        <%
         ...
         %>
          ...
          <td class=button align="center">
                ...
                <input type="button" name=buttonSave class='button' value="暂存" onclick="return saveFormNew(this,'CheckInput');">
      ...
      %>
       ...
       <%-- 并案暂时注释掉 <logic:notEqual name="prpLverifyLossDto" property="status" value="4">--%>
              <td class=button  align="center">
                <!--保存按钮-->
                 <input type="button" name=buttonSave class='button' value="暂存" onclick="return saveFormNew(this,'2');">
              </td>
            ...
            <input type=text name="nextHandlerName" class="codecode" style="width:80px" title="操作员" value=""
             ...
             onkeyup= "code_CodeSelect(this, 'HanderCode','-1','always','none','post');">-->
               ...
               <!--Modify by chenenda update end 20050420-->
                <input type="button" name=buttonSaveFinishSubmit class='button' value="<bean:message key='button.submit.value'/>" onclick="return saveFormNew(this,'4');" >
              </td>
              <td class=button  align="center">
                <!--取消按钮-->
                <input type=button name=buttonCancel class='button' value="<bean:message key='button.cancel.value'/>" onclick="return resetForm();" >
              </td>
          <%-- 并案暂时注释掉  </logic:notEqual>--%>
     <%}%>           
   <%-- 并案暂时注释掉  <logic:equal name="prpLverifyLossDto" property="status" value="4">--%>
<%
...
%>
      <td class=button  align="center">
        <!--取消按钮-->
         <input type="buttonBack" class="button" value="返 回" src="/claim/images/bgClaimBackButtonSmall.gif" align="middle" width="68" height="21"  border-style="none"  LANGUAGE=JAVASCRIPT  onmousedown="history.back();return false;">         
      </td>
<%
  }
%>
   <%-- 并案暂时注释掉 </logic:equal> --%>
    <%-- 并案暂时注释掉  <logic:equal name="prpLverifyLossDto" property="status" value="2">--%>
     	<td class=button  align="right">
        	 <input type="button" name=giveup class='button' value="放弃任务"  onclick="giveupCombineTemporarySave('certa');">
        </td>	 
    <%-- 并案暂时注释掉 </logic:equal>--%>
